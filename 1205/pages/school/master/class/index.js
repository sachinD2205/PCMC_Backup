...
const Index = () => {
  ...
  // Get Table - Data
  const getClassMaster = (_pageSize = 10, _pageNo = 0, _sortBy = "id", _sortDir = "desc") => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      const tempData = axios.post(`${urls.SCHOOL}/mstClass/save`, _body).then((res) => {
        if (res.status == 201) {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.SCHOOL}/mstClass/save`, body).then((res) => {
