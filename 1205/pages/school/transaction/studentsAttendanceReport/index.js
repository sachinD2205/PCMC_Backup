...
const Index = () => {
  ...
  const findReport = async (formValue) => {
    ...
    axios.post(`${urls.SCHOOL}/trnStudentAttendance/report`, _data).then((res) => {
      if (res.status == 200 || res.status == 201) {
        console.log("_attendanceRes", res.data);
        let a = res?.data;
        let _schoolData = {
          schoolName: schoolList?.find((i) => i?.id === a?.schoolKey)?.schoolName,
          schoolNameMr: schoolList?.find((i) => i?.id === a?.schoolKey)?.schoolNameMr,
          academicYearName: academicYearList?.find((i) => i?.id === a?.academicYearKey)?.academicYear,
          className: classList?.find((i) => i?.id === a?.classKey)?.className,
          divisionName: divisionList?.find((i) => i?.id === a?.divisionKey)?.divisionName,
          fromDate: moment(formValue.fromDate).format("YYYY-MM-DD"),
          toDate: moment(formValue.toDate).format("YYYY-MM-DD"),
        };
