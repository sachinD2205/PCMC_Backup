...
import styles from "../../../../components/streetVendorManagementSystem/styles/taxRateChart.module.css";
import schema from "../../../../components/streetVendorManagementSystem/schema/TaxRateChartSchema";
const Index = () => {
  const {
    register,
    control,
  ...
  const getUsageType = () => {
    axios.get(`${urls.PtaxURL}/usageType/getAllUsageType`).then((r) => {
      setusageTypes(
        r.data.map((row) => ({
          id: row.id,
  ...
  const getPropertyType = () => {
    axios.get(`${urls.PtaxURL}/propertyType/getPropertyTypeData`).then((r) => {
      setpropertyTypes(
        r.data.map((row) => ({
          id: row.id,
  ...
  const GetTaxRateChartMaster = () => {
    axios
      .get(`${urls.HMSURL}/taxRateChartMaster/getTaxRateChartMasterData`)
      .then((res) => {
        ...
        setDataSource(
          res.data.map((r, i) => ({
            ...
            usageType: usageTypes?.find((obj) => obj?.id === r.usageType)
              ?.usageType,
  ...
  const onSubmitForm = (formData) => {
    ...
    axios
      .post(
        `${urls.HMSURL}/taxRateChartMaster/saveTaxRateChartMaster`,
        finalBodyForApi,
      )
      .then((res) => {
        if (res.status == 201) {
  ...
  const deleteById = (value) => {
    swal({
      ...
      axios
        .delete(
          `${urls.HMSURL}/taxRateChartMaster/discardTaxRateChartMaster/${value}`,
        )
        .then((res) => {
          if (res.status == 226) {
            if (willDelete) {
