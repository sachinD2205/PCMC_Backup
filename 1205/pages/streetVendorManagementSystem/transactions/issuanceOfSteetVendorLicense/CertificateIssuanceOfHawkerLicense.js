...
const CertificateIssuanceOfHawkerLicense = () => {
  ...
  // issunce
  const getIssuanceHawkerCertificateData = () => {
  ...
  // renewal
  const getRenewalHawkerCertificateData = () => {
    axios
      .get(
        `${urls.HMSURL}/hawkerLiscenseCertificate/getByRenewalId?renewalOfliscenseId=${renewalOfHawkerLicenseId}`,
      )
      .then((r) => {
        if (r?.status == 200 || res?.status == 201 || res?.status == "SUCCESS") {
          setCertificateData(r?.data);
  ...
  // cancellation
  const getCancellationHawkerCertificateData = () => {
    axios
      .get(
        `${urls.HMSURL}/hawkerLiscenseCertificate/getById?issuanceOfliscenseId=${issuanceOfHawkerLicenseId}`,
      )
   ...
   // trnasfer
  const getTransferHawkerCertificateData = () => {
    axios
      .get(
        `${urls.HMSURL}/hawkerLiscenseCertificate/getById?issuanceOfliscenseId=${issuanceOfHawkerLicenseId}`,
      )
                  ...
                  <div className={styles.extraWidth1}>
                    <tr>
                      ...
                      <td colSpan={10} rowSpan={4} className={styles.borderBlack}>
                        <img
                          src={`${urls.CFCURL}/file/preview?filePath=${this?.props?.certificateData?.siteVisits[
