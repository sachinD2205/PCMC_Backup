...
const IdCardOfStreetVendor = () => {
  ...
  const getIssuanceICardData = () => {
   ...
   // renewal
  const getRenewalICardData = () => {
    setLoadderState(true);
    axios
      .get(`${urls.HMSURL}/hawkerLiscenseIdCard/getByRenewalId?renewalOfliscenseId=${renewalOfHawkerLicenseId}`)
      .then((r) => {
        if (r?.status == 200 || res?.status == 201 || res?.status == "SUCCESS") {
          setICardData(r?.data);
          setLoadderState(false);
    ...
    // cancellation
  const getCancellationICardData = () => {
    setLoadderState(true);
    axios
      .get(`${urls.HMSURL}/hawkerLiscenseIdCard/getById?issuanceOfliscenseId=${issuanceOfHawkerLicenseId}`)
      .then((r) => {
