...
const LoiGeneration = () => {
  ...
  const getIssuanceOfHawkerLicsenseData = () => {
   ...
   // Renewal
  const getRenewalOfHawkerLicsenseData = () => {
    axios
    ...
    // Cancellation
  const getCancellationOfHawkerLicsenseData = () => {
    axios
      .get(`${urls.HMSURL}/IssuanceofHawkerLicense/getById?id=${issuanceOfHawkerLicenseId}`)
      .then((r) => {
        if (r?.status == 200 || r?.status == 201 || r?.status == "SUCCESS") {
    ...
    // Transfer
  const getTransferOfHawkerLicsenseData = () => {
    axios
      .get(`${urls.HMSURL}/IssuanceofHawkerLicense/getById?id=${issuanceOfHawkerLicenseId}`)
      .then((r) => {
        if (r?.status == 200 || r?.status == 201 || r?.status == "SUCCESS") {
