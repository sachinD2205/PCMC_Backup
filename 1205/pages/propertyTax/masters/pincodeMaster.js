...
import Schema from "../../../containers/schema/propertyTax/masters/pincodeMaster";
...
const Index = () => {
  ...
  // Get Table - Data
  const getAllPincode = (_pageSize = 10, _pageNo = 0) => {
    axios
      ...
      .then((res) => {
        ...
        let _res = result?.map((val, i) => {
          ...
          return {
            ...
            srNo: i + 1,
            pinCode: val.pinCode,
            pinCodePrefix: val.pinCodePrefix,
  ...
  const onSubmitForm = (formData) => {
    ...
    axios.post(`${urls.PTAXURL}/master/pinCode/save`, finalBodyForApi).then((res) => {
      if (res.status == 200) {
        formData.id
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.PTAXURL}/master/pinCode/save`, body)
            .then((res) => {
  ...
  const columns = [
    ...
    { field: "pinCode", headerName: <FormattedLabel id="pincode" />, flex: 1 },
    {
      field: "actions",
      ...
      <div>
        <Paper style={{ margin: "30px" }}>
          ...
          >
            ...
            >
              <strong>
                <FormattedLabel id="pincodeMaster" />
              </strong>
            </Grid>
          </Box>
                  ...
                  >
                    ...
                    >
                      <TextField
                        size="small"
                        id="standard-basic"
                        label={<FormattedLabel id="pincodePrefix" />}
                        variant="standard"
                        {...register("pinCodePrefix")}
                        error={!!errors.pinCodePrefix}
                        helperText={errors?.pinCodePrefix ? errors.pinCodePrefix.message : null}
                      />
                    </Grid>
