...
import Schema from "../../../containers/schema/propertyTax/masters/subLocalityMaster";
...
const Index = () => {
  ...
  const [gatNames, setGatNames] = useState([{ id: 1, gatNo: 1, gatName: "Gat 1", gatNameMr: "Gat 1" }]);
  ...
  useEffect(() => {
    getAllLocality();
  }, [zones, circleName, gatNames, localitys]);

  console.log("222", zones);
  // Get Table - Data
  const getAllLocality = (_pageSize = 10, _pageNo = 0) => {
    axios
      .get(`${urls.PTAXURL}/master/subLocality/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        let result = res.data?.subLocality;
        let _res = result?.map((val, i) => {
          ...
          return {
            ...
            id: val.id,
            srNo: i + 1,
            subLocalityPrefix: val.subLocalityPrefix,
            administrativeZone: val.administrativeZone,
            administrativeZone1: zones?.find((obj) => obj.id === val.administrativeZone)?.adZoneName,
            circle: val.circle,
            gat: val.gat,
            gatName: gatNames.find((obj) => obj.id == val.gat)?.gatName,
            locality: val.locality,
            locality1: localitys?.find((obj) => obj.id === val.locality)?.locality,
  ...
  const getZone = () => {
    axios.get(`${urls.PTAXURL}/master/administrativeZone/getAll`).then((res) => {
      setZone(
  ...
  const onSubmitForm = (formData) => {
    ...
    axios
      .post(`${urls.PTAXURL}/master/subLocality/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.PTAXURL}/master/subLocality/save`, body)
            .then((res) => {
      ...
      <div>
        <Paper style={{ margin: "30px" }}>
          ...
          >
            ...
            >
              <strong>
                <FormattedLabel id="subLocalityMaster" />
              </strong>
            </Grid>
          </Box>
                  ...
                  >
                    ...
                    >
                      <TextField
                        size="small"
                        id="standard-basic"
                        label="SubLocality Prefix"
                        variant="standard"
                        {...register("subLocalityPrefix")}
                        error={!!errors.subLocalityPrefix}
                        helperText={errors?.subLocalityPrefix ? errors.subLocalityPrefix.message : null}
                      />
                    </Grid>
                    ...
                    >
                      <FormControl error={!!errors.administrativeZone} style={{ width: "230px" }}>
                        <InputLabel>{<FormattedLabel id="administrativeZone" />}</InputLabel>
                        <Controller
                          control={control}
                          render={({ field }) => (
                            <Select
                        ...
                        />
                        <FormHelperText>
                          {errors?.administrativeZone ? errors.administrativeZone.message : null}
                        </FormHelperText>
                      </FormControl>
                    </Grid>
                    ...
                    >
                      <FormControl error={!!errors.circleName} style={{ width: "230px" }}>
                        <InputLabel>{<FormattedLabel id="circleName" />}</InputLabel>
                        <Controller
                          control={control}
                          render={({ field }) => (
                            <Select
                    ...
                    >
                      <FormControl error={!!errors.gatName} style={{ width: "230px" }}>
                        ...
                        <Controller
                          ...
                          defaultValue=""
                        />
                        <FormHelperText>{errors?.gatName ? errors.gatName.message : null}</FormHelperText>
                      </FormControl>
                    </Grid>
                    ...
                    >
                      <FormControl error={!!errors.locality} style={{ width: "230px" }}>
                        <InputLabel>{<FormattedLabel id="locality" />}</InputLabel>
                        <Controller
                          control={control}
                          render={({ field }) => (
                            <Select
                              fullWidth
                              value={field.value}
                              label={<FormattedLabel id="locality" />}
                              onChange={(value) => field.onChange(value)}
                              // style={{ height: 40, padding: "14px 14px" }}
                              variant="standard"
                            >
                              {localitys &&
                                localitys.map((locality, i) => (
                                  <MenuItem key={i} value={locality.id}>
                                    {locality.locality}
                                  </MenuItem>
                                ))}
                            </Select>
                          ...
                          defaultValue=""
                        />
                        <FormHelperText>{errors?.locality ? errors.locality.message : null}</FormHelperText>
                      </FormControl>
                    </Grid>
                    ...
                    >
                      <TextField
                        ...
                        id="outlined-basic"
                        // label="Remark"
                        label={<FormattedLabel id="subLocality" />}
                        variant="standard"
                        {...register("subLocality")}
                        error={!!errors.subLocality}
                        helperText={errors?.subLocality ? errors.subLocality.message : null}
                      />
                    </Grid>
