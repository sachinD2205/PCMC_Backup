...
import Schema from "../../../containers/schema/propertyTax/masters/propertyGradingMaster";
...
const Index = () => {
  ...
  useEffect(() => {
    getAllPropertyGrading();
  }, []);

  // Get Table - Data
  const getAllPropertyGrading = (_pageSize = 10, _pageNo = 0) => {
    axios
      .get(`${urls.PTAXURL}/master/propertyGrading/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        ...
        let result = res.data?.propertyGrading;
        let _res = result?.map((val, i) => {
          ...
          return {
            ...
            id: val.id,
            srNo: i + 1,
            propertyGrading: val.propertyGrading,
            propertyGradingMr: val.propertyGradingMr,
  ...
  const onSubmitForm = (formData) => {
    ...
    axios
      .post(`${urls.PTAXURL}/master/propertyGrading/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.PTAXURL}/master/propertyGrading/save`, body)
            .then((res) => {
  ...
  const columns = [
    ...
    {
      field: "propertyGrading",
      headerName: <FormattedLabel id="propertyGrading" />,
      flex: 1,
    },
    {
      field: "propertyGradingMr",
      headerName: "Property Grading Mr",
      flex: 1,
    },
    {
      field: "fromDate",
      ...
      <div>
        <Paper style={{ margin: "30px" }}>
          ...
          >
            ...
            >
              <strong>
                <FormattedLabel id="propertyGradingMaster" />
              </strong>
            </Grid>
          </Box>
                  ...
                  >
                    ...
                    >
                      <TextField
                        ...
                        id="outlined-basic"
                        label={<FormattedLabel id="propertyGrading" />}
                        variant="standard"
                        {...register("propertyGrading")}
                        error={!!errors.propertyGrading}
                        helperText={errors?.propertyGrading ? errors.propertyGrading.message : null}
                      />
                    </Grid>
                    ...
                    >
                      <TextField
                        ...
                        style={{ backgroundColor: "white" }}
                        id="outlined-basic"
                        label="Property Grading Mr"
                        // variant="outlined"
                        variant="standard"
                        {...register("propertyGradingMr")}
                        error={!!errors.propertyGradingMr}
                        helperText={errors?.propertyGradingMr ? errors.propertyGradingMr.message : null}
                      />
                    </Grid>
          ...
          <Box style={{ height: "auto", overflow: "auto", padding: "10px" }}>
            <DataGrid
              ...
              columns={columns}
              onPageChange={(_data) => {
                getAllPropertyGrading(data?.pageSize, _data);
              }}
              onPageSizeChange={(_data) => {
                console.log("222", _data);
                // updateData("page", 1);
                getAllPropertyGrading(_data, data?.page);
              }}
            />
          </Box>
        </Paper>
