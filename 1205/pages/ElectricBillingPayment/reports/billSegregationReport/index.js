...
const Index = () => {
  ...
  const [billAmountDropDown, setBillAmountDropDown] = useState([]);
  const [consumedUnitDropDown, setConsumedUnitDropDown] = useState([]);
...
const getSubDivision = () => {
  ...
  const onSubmitFunc = (formData) => {
    ...
    console.log("apiBodyToSend",apiBodyToSend)
      ...
      setLoading(true);
      axios
        .post(`${urls.EBPSURL}/report/getBillSegregation`, apiBodyToSend)
        .then((res) => {
          console.log(":log", res);
        ...
        >
          ...
          >
            <strong className={styles.fancy_link1}>
              <FormattedLabel id="billSegregationReport" />
            </strong>
          </Box>
        </Box>
        {/* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> */}
