...
const Index = () => {
  ...
  useEffect(()=>{
     ...
     console.log("getChangeOfLoadData",_res);
     setValue("quantity", _res?.quantity ? _res?.quantity : "-");
     setValue("newQuantity", _res?.newQuantity ? _res?.newQuantity : "-");
     setValue("newCapacityKey", _res?.newCapacityKey ? _res?.newCapacityKey : "-");
     setValue("increaseLoad", _res?.increaseLoad ? _res?.increaseLoad : "-");
  ...
  useEffect(()=>{
    getChangeOfLoadDetails(router?.query?.id);
  },[router?.query?.id]);

  const getChangeOfLoadDetails = (id) => {
    if(id){
  ...
  const onSubmitForm = (formData) => {
    let _formData = {
      ...
      isApproved: null,
      isComplete: false,
      status: changeLoadData?.status,
      activeFlag: changeLoadData?.activeFlag,
  }
    const tempData = axios
      ...
      .then((res) => {
        if (res.status == 201) {
          sweetAlert("Update!", "Changed Load Updated Successfully !", "success");
