...
const Index = () => {
  ...
  const handleAddButton = () => {
          ...
          router.push('/ElectricBillingPayment/transaction/changeOfMeter/addNewMeter');
  }

  //handle view actions as per role
  const handleViewActions = (paramsRow) => {
      console.log("clicked id", paramsRow.status)
      if(authority && authority.find((val)=> val === "ENTRY") && paramsRow.status === "Rejected by Dy Eng")
      {
        router.push({
          pathname: '/ElectricBillingPayment/transaction/changeOfMeter/editMeterDetails',
          query: {
            id: paramsRow.id,
          },
        }
        ...
        router.push({
          pathname: '/ElectricBillingPayment/transaction/changeOfMeter/viewNewMeterDetails',
          query: {
            id: paramsRow.id,
          },
        }
  ...
  // Get Table - Data
  const getAllChangeOfMeterData = (_pageSize = 10, _pageNo = 0) => {
    console.log("_pageSize,_pageNo", _pageSize, _pageNo);

    axios
      .get(`${urls.EBPSURL}/trnChangeOfMeter/getAll`,
      {
        headers:{
      ...
      .then((r) => {
        let result = r.data.trnChangeOfMeterList;
        console.log("getAllChangeOfMeterData", result);
        ...
        if (department && consumptionType && billingDivisionAndUnit && subDivision) {
          let _res = tableData.map((r, i) => {
            ...
            return {
              ...
              consumerNoMr: r?.consumerNoMr,
              consumerName: r?.consumerName,
              consumerNameMr: r?.consumerNameMr,
              oldMeterNo: r?.oldMeterNo,
              newMeterNo: r?.newMeterNo,
              oldMeterClosingDate: moment(r?.oldMeterClosingDate).format("DD-MM-yyyy"),
                      ...
                      r.status == 4 ? "Rejected by Exe Eng" :
                        r.status == 5 ? "Waiting For Exe Eng" :
                          r.status == 6 ? "Rejected By Acc" :
                            ...
                            r.status == 25 ? "Waiting For Jr.Eng after approved" :
  ...
  const columns = [
    ...
    // consumerName
    {
      ...
      {
        field: "newMeterNo",
        headerName: <FormattedLabel id="newMeterNo" />,
        headerAlign: "center",
        align: "center",
        width: 150,
      },
        ...
        {
          field: "oldMeterNo",
          headerName: <FormattedLabel id="oldMeterNo" />,
          headerAlign: "center",
          align: "center",
          width: 150,
        },
         ...
         // oldCloseDate
         {
          field: "oldMeterClosingDate",
          headerName: <FormattedLabel id="oldMeterClosingDate" />,
          headerAlign: "center",
          align: "center",
          width: 150,
        },
    ...
    // status
    {
      ...
      headerName: <label style={{alignItems:"center"}}>Status</label>,
      flex: 1,
      renderCell: (params) => {
        return (
          <Box>
           {
            ...
            :
              params.row.status.includes("Waiting for Dy") && authority && authority[0] === "DEPUTY_ENGINEER" ? 
              <p style={{color:"blue"}}>{params.row.status}</p>
            : 
            params.row.status.includes("Waiting For Ex") && authority && authority[0] === "EXECUTIVE_ENGINEER" ? 
            <p style={{color:"blue"}}>{params.row.status}</p>
          : 
  ...
  return (
    ...
    >
      ...
      <DataGrid
        ...
        columns={columns}
        onPageChange={(_data) => {
          getAllChangeOfMeterData(data.pageSize, _data);
        }}
        onPageSizeChange={(_data) => {
          console.log("222", _data);
          // updateData("page", 1);
          getAllChangeOfMeterData(_data, data.page);
        }}
      />
