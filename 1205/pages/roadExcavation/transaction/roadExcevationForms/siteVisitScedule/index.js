...
import { Button, FormControl, FormHelperText, IconButton, Modal, TextField, Typography } from "@mui/material";
...
const SiteVisitSchedule = ({ appID }) => {
  ...
  const appendUI = (id, fromTime, toTime, noOfSlots) => {
    console.log("id", id);
    console.log(`sdf ${getValues(`slots.length`)}`);
      ...
      axios
        ...
        .then((r) => {
          if (r?.status == 200 || res?.status == 201 || res?.status == "SUCCESS") {
            ...
            router.push("/roadExcavation/dashboard");
            console.log("res", r);
          } else {
            setLoadderState(false);
            // <Failed />;
  ...
  const getSlot = (selectedDate) => {
    ...
    axios
      ...
      .then((r) => {
        if (r?.status == 200 || res?.status == 201 || res?.status == "SUCCESS") {
          setLoadderState(false);
        //   console.log("rrrrrrrrrrr",selectedDate);
        ...
        <ThemeProvider theme={theme}>
          ...
          >
            <strong>
                ...
                Site Visit Appointment Schedule
                </strong>
          </div>
          ...
          >
            <form onSubmit={handleSubmit(onFinish)}>
              <div className={styles.box}>
                <div className={styles.titlemodelT} style={{ marginLeft: "25px" }}>
                  <Typography
                  ...
                  >
                    ...
                    Add Slots Of Site Visit
                  </Typography>
                  <IconButton onClick={() => router.push(`/roadExcavation/dashboard`)}>
                    <CloseIcon />
                  </IconButton>
                </div>
                ...
                <div container style={{ overflowY: "auto", padding: "10px", backgroundColor: "#F9F9F9" }}>
                        ...
                        <div className={styles.row1}>
                          ...
                          <div style={{ marginLeft: "50px" }}>
                            <TextField
                              ...
                              variant="standard"
                              {...register(`slots.${index}.noOfSlots`)}
                            />
                            <TextField
                              hidden
                              sx={{ opacity: "0%" }}
                              key={slot.id}
                              {...register(`slots.${index}.id`)}
                            />
                          </div>
                          <div
                            item
                          ...
                          >
                            {!haveData ? (
                              ...
                              <Button
                                ...
                                //   setLoadderState(true);
                                  let data = {
                                    slotId: getValues(`slots.${index}.id`),
                                    applicantId: router.query.id,
                                    // tokenNo: getValues("tokenNo"),
                                    fromTime: moment(getValues(`slots.${index}.fromTime`), "HH:mm:ss a").format("HH:mm:ss"),
                                    toTime: moment(getValues(`slots.${index}.toTime`), "HH:mm:ss a").format("HH:mm:ss")
                                  };
