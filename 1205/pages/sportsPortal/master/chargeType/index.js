...
import schema from "../../../../containers/schema/sportsPortalSchema/chargeTypeSchema";
...
const Index = () => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      const tempData = axios.post(`${URLS.SPURL}/chargeType/save`, _body).then((res) => {
        if (res.status == 200) {
    ...
    // Update Data Based On ID
      ...
      axios.post(`${URLS.SPURL}/chargeType/saveFacilityName`, _body).then((res) => {
        console.log("res", res);
        if (res.status == 200) {
          fromData.id
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${URLS.SPURL}/chargeType/saveFacilityName`, body).then((res) => {
  ...
  return (
    <>
      <Paper sx={{ marginLeft: 5, marginRight: 5, marginTop: 5, marginBottom: 5, padding: 1 }}>
        ...
        >
          <strong>{<FormattedLabel id="chargeTypeMaster" />}</strong>
        </div>
        {isOpenCollapse && (
                ...
                <form onSubmit={handleSubmit(onSubmitForm)}>
                  <div className={styles.main}>
                    <div className={styles.row}>
                      <div>
                        <FormControl style={{ marginTop: 10 }} error={!!errors.typeName}>
                          <TextField
                            id="standard-basic"
                            label={<FormattedLabel id="chargeTypeEn" />}
                            // label="Charge Type"
                            variant="standard"
                            sx={{ width: 220 }}
                            {...register("typeName")}
                          />
                          <FormHelperText>{errors?.typeName ? errors.typeName.message : null}</FormHelperText>
                        </FormControl>
                      </div>
                      <div>
                        <FormControl style={{ marginTop: 10 }} error={!!errors.typeNameMr}>
                          <TextField
                            id="standard-basic"
                            label={<FormattedLabel id="chargeTypeMr" />}
                            // label="Charge Type (MR)"
                            variant="standard"
                            sx={{ width: 220 }}
                            {...register("typeNameMr")}
