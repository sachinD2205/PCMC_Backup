...
const Index = () => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      const tempData = axios.post(`${URLS.SPURL}/applicantType/save`, _body).then((res) => {
        if (res.status == 200) {
    ...
    // Update Data Based On ID
      ...
      axios.post(`${URLS.SPURL}/applicantType/save`, _body).then((res) => {
        console.log("res", res);
        if (res.status == 200) {
          fromData.id
  ...
  // Get Table - Data
  const getType = (_pageSize = 10, _pageNo = 0) => {
                ...
                <form onSubmit={handleSubmit(onSubmitForm)}>
                  <div className={styles.main}>
                    <div className={styles.row}>
                      <div>
                        <FormControl variant="standard" error={!!errors.typeName}>
                          <TextField
                            id="standard-basic"
                      ...
                      <div>
                        <FormControl variant="standard" error={!!errors.typeNameMr}>
                          <TextField
                            id="standard-basic"
                            label={<FormattedLabel id="applicantTypeMr" />}
                            // label="Applicant Type (MR)"
                            variant="standard"
                            sx={{ width: 220 }}
                            {...register("typeNameMr")}
                          />
