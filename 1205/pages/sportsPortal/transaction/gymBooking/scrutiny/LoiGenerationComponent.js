...
const LoiGenerationComponent = (props) => {
  ...
  const getServiceCharges = () => {
    axios.get(`${urls.CFCURL}/master/servicecharges/getByServiceId?serviceId=${36}`).then((r) => {
  ...
  const getLoiGenerationData = () => {
    axios.get(`${urls.SPURL}/gymBooking/getById?id=${props?.data?.id}`).then((r) => {
      if (r.status === 200) {
  ...
  const handleNext = (data) => {
    ...
    axios.post(`${urls.SPURL}/gymBooking/saveApplicationApprove`, finalBodyForApi).then((res) => {
      if (res.status == 200) {
        ...
        router.push({
          pathname: "/sportsPortal/transaction/gymBooking/scrutiny/LoiGenerationReciptmarathi",
          query: {
