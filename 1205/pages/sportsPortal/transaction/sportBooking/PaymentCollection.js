...
const Index = (props) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${urls.SPURL}/sportsBooking/getById?id=${props?.data?.id ? props?.data?.id : localStorage.getItem("id")}`).then((res) => {
      reset(res.data);
      ...
      console.log("board data12121", res.data);
    });
    // }
  }, []);
  ...
  const handlePay = () => {
    ...
    console.log("Slot chi ID", router?.query?.bookingId);
    const finalBody = {
      bookingId: Number(router?.query?.bookingId) ? Number(router?.query?.bookingId) : dataa?.bookingId,
      paymentCollection: { ...getValues("payment"), paymentMode: paymentModes?.find((p) => p?.paymentMode == getValues("payment.paymentMode"))?.id, },
      id: getValues("id"),
    };
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          <div className={styles.appDetails}>
            ...
            {/* <h4>एकुण रक्कम : {dataa?.finalTotal} रु</h4> */}
                      ...
                      <b>
                        {applicableCharages.map((charge, index) => {
                          return (
                            <td colSpan={4}>
                              {" "}
                              = {charge.amountPerHead} * {dataa?.totalGroupMember}
                            </td>
                          );
                        })}
                      </b>
