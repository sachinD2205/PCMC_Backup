...
const LoiGenerationComponent = (props) => {
  ...
  const getLoiGenerationData = () => {
    axios.get(`${urls.SPURL}/groundBooking/getById?id=34`).then((r) => {
      if (r.status === 200) {
  ...
  //   // })
  //   if (data) {
  //     setValue("loi.amount", Number(data?.serviceCharge + data?.penaltyCharge));

  //     setValue("loi.totalInWords", toWords.convert(data?.serviceCharge + data?.penaltyCharge));
  //   }
  // }, [data]);
