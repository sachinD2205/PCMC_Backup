...
const LinaerStepper = () => {
  ...
  const handleNext = (data) => {
    ...
    let _body = {
      ...
      fromBookingTime: moment(data.fromBookingTime, "YYYY-MM-DDTHH:mm:ss"),
      toBookingTime: moment(data.toBookingTime, "YYYY-MM-DDTHH:mm:ss"),

      // toBookingTime: moment(data.toBookingTime, "YYYY-MM-DDT:HH:MM:SS A").format("YYYY-MM-DDT:HH:MM:SS A"),
    ...
    if (activeStep == steps.length - 1) {
      axios.post(`${URLS.SPURL}/groundBooking/saveGroundBooking`, _body).then((res) => {
        if (res.status == 200) {
