...
const Index = (props) => {
  ...
  const handlePay = () => {
    ...
    axios
      .post(`${urls.SPURL}/groundBooking/saveApplicationApprove`, finalBody)
      .then((res) => {
        console.log("98765654", res);
        swal("Submitted!", "Payment Collected successfully !", "success");
        router.push({
          pathname: "/sportsPortal/transaction/groundBookingNew/scrutiny/ServiceChargeRecipt",
          query: {
  ...
  useEffect(() => {
    axios.get(`${urls.SPURL}/groundBooking/getById?id=${props?.data?.id}`).then((res) => {
      reset(res.data);
      setDataa(res.data);
      console.log("board data", res.data.applicationStatus);
      console.log("67576556", res);
    });
  }, []);
