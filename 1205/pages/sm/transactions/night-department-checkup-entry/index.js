...
function NightDepartmentCheckupEntry() {
  ...
  const getEntryByEmployeeDetails = () => {
    // Filter through employee list to get the selected employee details based on employee id
    let selectedEmployee = employee.filter((item) => item.empCode === entryEmployeeId);
    console.log("selectedEmployee", selectedEmployee);

    // setValue("mobileNumber", selectedEmployee[0]?.phoneNo);
    setValue("entryBy", selectedEmployee[0]?.firstNameEn + " " + selectedEmployee[0]?.lastNameEn);
  };
                  ...
                  <Grid item xs={6}>
                    <Controller
                      ...
                      render={({ field }) => (
                        <TextField
                          {...field}
                          label={<FormattedLabel id="presentEmployeeMobile" required />}
                          InputLabelProps={{
                            shrink: true,
                          }}
                          size="small"
