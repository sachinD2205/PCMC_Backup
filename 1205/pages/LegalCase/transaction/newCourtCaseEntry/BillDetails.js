...
const BillDetails = () => {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("newCourtCaseEntry") !== "null") {
      setCourtCaseEntryData(JSON.parse(localStorage.getItem("newCourtCaseEntry")));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    let pendingAmt = courtCaseEntryData?.pendingAmount;
    let caseFees = paymentRates?.find((item) => item?.caseSubType === getValues("caseSubTypeId"))?.rate;
    // let A = dataSource?.find((item) => item?.caseSubType === getValues("caseSubTypeId"))?.pendingFees;
    let totalPaidFees = dataSource?.reduce((total, obj) => total + obj?.paidFees, 0);
      ...
      <Grid container style={{ marginLeft: 70, padding: "10px" }}>
        ...
        <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
          <TextField
            ...
            helperText={errors?.pendingFees ? errors?.pendingFees?.message : null}
            InputLabelProps={{
