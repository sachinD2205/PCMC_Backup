...
const View = () => {
  ...
  useEffect(() => {
    if (
      ...
      axios.get(`${urls.LCMSURL}/transaction/opinion/getById?id=${router?.query?.id}`).then((res) => {
  ...
  const onSubmitForm = (data) => {
    ...
    const bodyForAPIOpinion = {
      ...
      opinionMr: data?.panelAdvocateOpinionMr ? data?.panelAdvocateOpinionMr : "-",
      ...
      //activeFlag: "Y",
      trnOpinionAttachmentDao: JSON.parse(localStorage.getItem("trnOpinionAttachmentDao")),
    };
    ...
    const bodyForAPISearchReport = {
      ...
      //remarkMr: data?.reportTitleAdvocateOpinionMr ? data?.reportTitleAdvocateOpinionMr : null,
      //remark: data?.reportTitleAdvocateOpinion ? data?.reportTitleAdvocateOpinion : null,
      opinionMr: data?.reportTitleAdvocateOpinionMr ? data?.reportTitleAdvocateOpinionMr : "-",
      opinion: data?.reportTitleAdvocateOpinion ? data?.reportTitleAdvocateOpinion : "-",
      //reportDate: moment(new Date()).unix().toString(),
      //submissionDate: moment(new Date()).unix().toString(),
      //status: "ADD_OPINION_SUBMITTED",
      //activeFlag: "Y",
    ...
    const bodyForAPI = {
      ...
      opinionAdvPanelList: data?.opinionAdvLst?.map((rpt) => {
        if (rpt.advocate == getValues("opinionAdvPanelList")) {
          return {
            ...rpt,
            remark: data?.panelAdvocateOpinion ? data?.panelAdvocateOpinion : null,
            remarkMr: data?.panelAdvocateOpinionMr ? data?.panelAdvocateOpinionMr : null,
          };
        }
      }),
    ...
    if (router.query.reportAdvPanel && getValues("reportAdvPanelList")?.includes(user.userDao.advocateId)) {
      console.log("bodyForAPISearchReport", bodyForAPISearchReport);
      axios
        .post(`${urls.LCMSURL}/reportAdvocatePanel/save`, bodyForAPISearchReport, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        ...
        .catch((err) => {
          console.log("bodyForAPISearchReport err", err);
          sweetAlert(err.message, "Something went wrong !", "error");
          console.log("err", err);
        });
    }

    if (router.query.advPanel && getValues("opinionAdvPanelList")?.includes(user.userDao.advocateId)) {
        ...
        return (
          <>
            <IconButton
              color="primary"
              onClick={() => {
                window.open(`${urls.CFCURL}/file/preview?filePath=${record.row.fileName}`, "_blank");
              }}
            >
              <Visibility />
            </IconButton>

            <IconButton color="error" onClick={() => discard(record.row.fileName, record.row.srNo)}>
              <Delete />
            </IconButton>
          </>
        );
              ...
              <Grid container sx={{ padding: "10px" }}>
                ...
                <Grid item xl={3} lg={3} md={6} sm={6} xs={12}>
                  <FormControl variant="standard">
                            ...
                            renderInput={(params) => (
                              <TextField
                                ...
                                InputLabelProps={{
                                  ...
                                  //true
                                  shrink:
                                    (watch("searchTitleRptDate") ? true : false) ||
                                    (router.query.searchTitleRptDate ? true : false),
                                }}
                              />
                            )}
                          />
                    ...
                    />
                    <FormHelperText>
                      {errors?.searchTitleRptDate ? errors.searchTitleRptDate.message : null}
                    </FormHelperText>
                  </FormControl>
                </Grid>
              </Grid>
                    ...
                    getValues("opinionAdvPanelList")?.includes(user.userDao.advocateId) && (
                      <>
                        <TextField
                          ...
                          helperText={errors?.opinion ? errors.panelRemarks.message : null}
                        />
                      </>
                    )}
                </Grid>
                    ...
                    <Grid container sx={{ padding: "10px" }}>
                      ...
                      <Grid item xs={12} xl={12} md={12} sm={12}>
                        <TextField
                          ...
                          helperText={
                            errors?.panelAdvocateOpinion ? errors.panelAdvocateOpinion.message : " "
                          }
                        />
                      </Grid>
                      ...
                      <Grid item xs={12} xl={12} md={12} sm={12}>
                        <TextField
                          ...
                          InputLabelProps={{
                            //true
                            shrink:
                              (watch("panelAdvocateOpinionMr") ? true : false) ||
                              (router.query.panelAdvocateOpinionMr ? true : false),
                          }}
                          error={!!errors.panelAdvocateOpinionMr}
                          helperText={
                            errors?.panelAdvocateOpinionMr ? errors.panelAdvocateOpinionMr.message : " "
                          }
                        />
                      </Grid>
                    </Grid>
                    ...
                    <Grid container sx={{ padding: "10px" }}>
                      <Grid item xs={12} xl={12} md={12} sm={12}>
                        <TextField
                          id="standard-textarea"
                          disabled={router?.query?.pageMode === "View"}
                          label={<FormattedLabel id="reportAdvocateOpinionEn" />}
                          // label="Advocate Opinion"
                          multiline
                          variant="standard"
                          ...
                          InputLabelProps={{
                            //true
                            shrink:
                              (watch("reportTitleAdvocateOpinion") ? true : false) ||
                              (router.query.reportTitleAdvocateOpinion ? true : false),
                          }}
                          // InputLabelProps={{
                          //   shrink: //true
                      ...
                      >
                        <TextField
                          ...
                          disabled={router?.query?.pageMode === "View"}
                          // label="Opinion"
                          label={<FormattedLabel id="reportAdvocateOpinionMr" />}
                          ...
                          InputLabelProps={{
                            //true
                            shrink:
                              (watch("reportTitleAdvocateOpinionMr") ? true : false) ||
                              (router.query.reportTitleAdvocateOpinionMr ? true : false),
                          }}
              ...
              <div className={styles.small}>
                ...
                <div className={styles.details}>
                  <div className={styles.h1Tag}>
                    ...
                    >
                      (Only jpg.jpeg.png.bmp.pdf file allowed to upload. File size should be 2 MB)
                    </h5>
                  </div>
                </div>
