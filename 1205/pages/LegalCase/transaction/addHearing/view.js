...
import { Typography, TextField, Button, Stepper, Step, StepLabel, Paper, Box } from "@mui/material";
...
import { addHearingSchema, schema } from "../../../../containers/schema/LegalCaseSchema/addHearingSchema";
...
const View = () => {
  ...
  function getSteps() {
    return [<FormattedLabel key={1} id="addHearing" />, <FormattedLabel key={2} id="document" />];
  }
  ...
  const handleNext = (data) => {
    ...
    if (activeStep == steps.length - 1) {
      axios.post(`${urls.LCMSURL}/trnsaction/addHearing/save`, finalBody).then((res) => {
        if (res.status == 200) {
              ...
              <form onSubmit={methods.handleSubmit(handleNext)}>
                ...
                <Box sx={{ display: "flex", flexDirection: "row", pt: 2 }}>
                  ...
                  <Button
                    variant="contained"
                    color="primary"
                    onClick={() => router.push(`/LegalCase/transaction/newCourtCaseEntry/`)}
                  >
