...
const Index = () => {
  ...
  const getNewMarriageRegistractionDetails = () => {
    ...
    axios
      .get(
        `${urls.SSLM}/Trn/ApplicantDetails/getByServicId?serviceId=7`,

      )
      .then((res) => {
        console.log(res, "reg123")
        setTableData(
          res.data.trnApplicantDetailsDao
            .map((r, i) => {
              return {
                srNo: i + 1,
                ...r,
  ...
  const saveApproval = (body) => {
    axios
      .post(
        `${urls.SSLM}/Trn/ApplicantDetails/saveApplicationApprove`, body)
      .then((response) => {
        if (response.status === 200 || response.status == 201) {
          // if (body.role === 'LOI_GENERATION') {
          router.push({
            pathname:
              '/skySignLicense/report/businessCertificateReport',
            query: {
              ...body,
            },
          })
          ...
          <>
            <div className={styles.buttonRow}>
                ...
                (authority?.includes('DOCUMENT_CHECKLIST') ||
                  ...
                  <IconButton
                    onClick={() =>
                      router.push({
                        pathname:
                          '/skySignLicense/transactions/issuanceOfBusinessOrIndustry/scrutiny/scrutiny',
                        query: {
                          disabled: true,
                          ...record.row,
              ...
              {record?.row?.applicationStatus ===
                ...
                (authority?.includes('APPOINTMENT_SCHEDULE') || authority?.includes('DOCUMENT_CHECKLIST') ||
                  authority?.includes('ADMIN')) && (
                  <IconButton>
                    <Button
                      variant="contained"
                  ...
                  <IconButton>
                    <Button
                      ...
                      onClick={() =>
                        router.push({
                          pathname:
                            '/skySignLicense/transactions/issuanceOfBusinessOrIndustry/LoiGenerationComponent',
                          query: {
                            id: record.row.id,
                  ...
                  <IconButton>
                    <Button
                      ...
                      onClick={() =>
                        router.push({
                          pathname:
                            '/skySignLicense/transactions/issuanceOfBusinessOrIndustry/PaymentCollection',
      ...
      >
        ...
        <div className={styles.detailsTABLE}>
          <div className={styles.h1TagTABLE}>
            ...
            >
              {' '}
              {<FormattedLabel id="businessLicense" />}
