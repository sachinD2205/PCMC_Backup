...
import CancellationofIndustry from "../components/CancellationofIndustry.js";
...
const LinaerStepper = () => {
  ...
  const { errors, getValues, setValue, reset, control, register, handleSubmit } = methods
  ...
  useEffect(() => {
    reset(router?.query);
    if (router.query.id) {
      axios.get(`${urls.SSLM}/Trn/ApplicantDetails/getApplicationById?appId=${router.query.id}`,)
        .then((r) => {
