        ...
        setDataSource(
          r.data.map((r, i) => {
            return {
              srNo: i + 1,
              ...r,
              greligionByBirth: religion?.find(
                (rel) => r.greligionByBirth == rel?.id,
              )?.religion,
              greligionByBirthMr: religion?.find(
                (rel) => r.greligionByBirth == rel?.id,
              )?.religionMr,

              breligionByBirth: religion?.find(
                (rel) => r.breligionByBirth == rel?.id,
              )?.religion,
              breligionByBirthMr: religion?.find(
                (rel) => r.breligionByBirth == rel?.id,
              )?.religionMr,

              greligionByAdoption: religion?.find(
                (rel) => r.greligionByAdoption == rel?.id,
              )?.religion,
              greligionByAdoptionMr: religion?.find(
                (rel) => r?.greligionByAdoption == rel?.id,
              )?.religionMr,

              breligionByAdoption: religion?.find(
                (rel) => r.breligionByAdoption == rel?.id,
              )?.religion,
              breligionByAdoptionMr: religion?.find(
                (rel) => r?.breligionByAdoption == rel?.id,
              )?.religionMr,

              gmaritalStatus: maritalStatus?.find(
                (rel) => r?.gstatusAtTimeMarriageKey == rel?.id,
              )?.statusDetails,
              gmaritalStatusMr: maritalStatus?.find(
                (rel) => r?.gstatusAtTimeMarriageKey == rel?.id,
              )?.statusDetailsMar,

              bmaritalStatus: maritalStatus?.find(
                (rel) => r?.bstatusAtTimeMarriageKey == rel?.id,
              )?.statusDetails,
              bmaritalStatusMr: maritalStatus?.find(
                (rel) => r?.bstatusAtTimeMarriageKey == rel?.id,
              )?.statusDetailsMar,
            }
          }),
                ...
                <thead>
                  ...
                  <tr>
                    <th colSpan={30}>
                      <h3>
                        <b>
                          नमुना आय (विवाह नोंदवही )<br /> (पहा कलम ६(१)(ई),८ आणि
                          नियम ९) सालाकरिता विवाह नोंदवही
                        </b>
                      </h3>
                    </th>
                  </tr>
...
export default ApplicationFormReport
