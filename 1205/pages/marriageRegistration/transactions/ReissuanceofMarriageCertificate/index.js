...
// import ApplicationInformation from "../../../../components/marriageRegistration/ApplicationInformation";
...
const Index = () => {
  ...
  const validateSearch = () => {
    console.log("22", "Bride Name validate", watch("marriageDate"), watch("registrationDate"));
  ...
  const handleApply = () => {
    ...
    axios
      .post(`${urls.MR}/transaction/reIssuanceM/saveReIssuanceOfMarriageRegistration`, finalBody)
      .then((res) => {
        if (res.status == 201) {
          ...
          router.push({
            pathname: "/marriageRegistration/transactions/ReissuanceofMarriageCertificate/PaymentCollection",
            ...
            {router?.query?.pageMode == "View" ? (
              ...
              <>
                            ...
                            render={({ field }) => (
                              <LocalizationProvider dateAdapter={AdapterMoment}>
                                <DatePicker
                                  inputFormat="DD/MM/YYYY"
                                  label={
                                    <span style={{ fontSize: 14 }}>{<FormattedLabel id="regDate" />}</span>
                                  }
                                  value={field.value}
                                  onChange={(date) => field.onChange(moment(date).format("YYYY-MM-DD"))}
                                  selected={field.value}
                ...
                >
                  ...
                  >
                    <Typography>
                      2) Filter (Zone * ,marriage date *,groomFName ,groomLName ,brideFName ,brideLName)
                    </Typography>
                  </AccordionSummary>
                  <AccordionDetails>
                    <div className={styles.row}>
                      ...
                      render={({ field }) => (
                        <LocalizationProvider dateAdapter={AdapterMoment}>
                          <DatePicker
                            disabled
                            inputFormat="DD/MM/YYYY"
                            label={<span style={{ fontSize: 14 }}>{<FormattedLabel id="mDate" />}</span>}
                            value={field.value}
                            onChange={(date) => field.onChange(moment(date).format("YYYY-MM-DD"))}
                            selected={field.value}
                            center
