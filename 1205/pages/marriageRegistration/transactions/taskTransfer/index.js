...
const Index = () => {
  ...
  const getTaskTransferForm = () => {
    axios.get(`${urls.MR}/TaskTransfer/getTaskTransferDetails`).then((resp) => {
      setDataSource(
  ...
  const editForm = async () => {
    ...
    if (okText == 'Save') {
      await axios
        .put(`${urls.MR}/TaskTransfer/updateTaskTransfer`, parametersForEdit)
        .then((resp) => {
          if (resp.status == 200) {
  ...
  const deleteForm = async (record) => {
    ...
    await axios
      .delete(`${urls.MR}/TaskTransfer/deleteTaskTransfer/${recordId}`)
      .then((resp) => {
        if (resp.status == 200) {
        ...
        <Card>
          <div style={{ display: 'flex', justifyContent: 'right' }}>
            <Button
              ...
              onClick={() => {
                router.push(
                  `/marriageRegistration/transactions/taskTransfer/view`
                )
              }}
            >
              {' '}
                ...
                <Col xl={7} lg={7} md={7} sm={7} xs={24}>
                  ...
                  >
                    <DatePicker
                      disabled={inputState}
                      defaultValue={
                        dataInModal && moment(dataInModal.toDuration)
                      }
                      onChange={(e) => {
                        setDataInModal((preDataInModal) => ({
                          ...preDataInModal,
