...
const Index = () => {
  ...
  useEffect(() => {
    axios.get(`${urls.CFCURL}/master/servicecharges/getByServiceId?serviceId=10`).then((r) => {
  ...
  const handlePay = () => {
    ...
    axios
      .post(`${urls.MR}/transaction/applicant/saveApplicationApprove`, finalBody)
      .then((res) => {
        console.log(res);
        swal("Submitted!", "Payment Collected successfully !", "success");
        router.push({
          pathname: "/marriageRegistration/transactions/newMarriageRegistration/scrutiny/ServiceChargeRecipt",
          query: {
            ...router?.query,
          },
        });
    ...
    if (router?.query?.serviceId == 67) {
      ...
      axios
        ...
        .then((res) => {
          ...
          setDataa(res.data);
          console.log("board data", res.data);
          setValue("payment.receiptAmount", res.data.payment.amount);
        });
