  ...
  const handleNext = (data) => {
    ...
    if (activeStep == steps.length - 1) {
      ...
      if (router?.query?.pageMode != "View") {
        axios.post(`${urls.MR}/transaction/applicant/saveApplicantRegistration`, finalBody).then((res) => {
          if (res.status == 201) {
      ...
      <Dialog fullWidth maxWidth={"lg"} open={formPreviewDailog} onClose={() => formPreviewDailogClose()}>
        ...
        <DialogTitle>
          <Grid container>
            ...
            <Grid item xs={1} sm={2} md={4} lg={6} xl={6} sx={{ display: "flex", justifyContent: "center" }}>
              <IconButton
                aria-label="delete"
                sx={{
                  marginLeft: "530px",
