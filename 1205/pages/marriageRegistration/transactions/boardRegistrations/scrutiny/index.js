...
const Index = () => {
    ...
    axios
      ...
      .then((resp) => {
        ...
        if (authority?.find((r) => r === "DOCUMENT_VERIFICATION" || authority?.find((r) => r === "ADMIN"))) {
          ...
          clkVerified = resp.data.filter((data) =>
            [
  ...
  const saveApproval = (body) => {
    axios
      .post(`${urls.MR}/transaction/marriageBoardRegistration/saveMarraigeBoardRegistrationApprove`, body, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      })
      .then((response) => {
        if (response.status === 200) {
          ...
          } else if (body.role === "CERTIFICATE_ISSUER" || body.role === "APPLY_DIGITAL_SIGNATURE") {
            router.push({
              ...
              {["APPLICATION_CREATED", "CITIZEN_SEND_TO_JR_CLERK"].includes(record?.row?.applicationStatus) &&
                (authority?.includes("DOCUMENT_CHECKLIST") || authority?.includes("ADMIN")) && (
                  <IconButton
                    onClick={() =>
                      router.push({
                        pathname: "/marriageRegistration/transactions/boardRegistrations/scrutiny/scrutiny",
                        query: {
                          disabled: true,
                          ...record.row,
