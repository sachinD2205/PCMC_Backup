...
const Index = (props) => {
    ...
    const onSubmitForm = (data) => {
        ...
        axios
            .post(`${urls.LMSURL}/libraryMembership/cancellationMembership?membershipNo=${watch('membershipNo')}&cancellationRemark=${watch('remark')}&closeDate=${moment(watch('closeDate')).format("YYYY/MM/DD")}`,)
            .then((res) => {
                if (res.status == 200 || res.status == 201) {
                    swal('Closed!', 'Membership Closed successfully !', 'success')
                    router.push({
            ...
            .catch((err) => {
                swal('Error!', 'Somethings Wrong Membership not Closed!', 'error')
            })
