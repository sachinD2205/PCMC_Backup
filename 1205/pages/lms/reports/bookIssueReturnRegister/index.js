...
const Index = () => {
  ...
  useEffect(() => {
    if (watch('fromDate') && watch('toDate')) {
      ...
      axios
        .get(
          `${urls.LMSURL}/trnBookIssueReturn/bookIssued?fromDate=${moment(watch('fromDate')).format('YYYY-MM-DD')}&toDate=${moment(watch('toDate')).format('YYYY-MM-DD')}`,
