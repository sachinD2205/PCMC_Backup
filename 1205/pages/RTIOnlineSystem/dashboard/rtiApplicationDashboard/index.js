...
const Index = () => {
    ...
    const user1 = useSelector((state) => {
        ...
        let userNamed = language === "en" ? state?.user?.user?.userDao && state?.user?.user?.userDao.firstNameEn :state?.user?.user?.userDao&& state?.user?.user?.userDao.firstNameMr;
        return userNamed;
    });
                ...
                return (
                    <>
                        {params?.row?.statusVal === 3 ? (
                            <div style={{ color: params?.row?.noOfDays<=5?"red":"blue"}}>
                                {params?.row?.status}
                            </div>
                        ) : (
                            ...
                            ) : (<div style={{ color: params?.row?.noOfDays<=5?"red":"black" }}>
                                {params?.row?.status}
                            </div>)))
                        )}
                    </>
            ...
            renderCell: (params) => {
                return (
                    <>
                        ...
                        >
                            <Tooltip title={language=="en"?`View Application against  ${params?.row?.applicationNo}`:`${params?.row?.applicationNo} अर्ज पहा`}>
                        ...
                        {(authority && authority?.find((val) => val === "RTI_ADHIKARI") && params.row.statusVal === 3 && !params.row.isTransfer && !params.row.parentId) && (<IconButton
                            onClick={() => {
                                sweetAlert({
                                    ...
                                    text: language=="en"?"Do you want to Transfer/Forward Application to another department?":"तुम्हाला अर्ज दुसऱ्या विभागात हस्तांतरित/फॉरवर्ड करायचा आहे का?",
                                    dangerMode: false,
                ...
                <Grid container style={{ padding: "10px" }}>
                    ...
                    >
                        <Button
                            ...
                            onClick={() => {
                                router.push('/RTIOnlineSystem/transactions/rtiApplication')
                            }}
                        >
