...
const Index = () => {
  ...
  // Get Table - Data
  const getBonafideCertificateMaster = (_pageSize = 10, _pageNo = 0) => {
    // console.log("_pageSize,_pageNo", _pageSize, _pageNo);
    axios
      .get(`${urls.SCHOOL}/trnBonafiedCertificate/getAllKK`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((r) => {
        let result = r.data.trnItiBonafiedCertificateList;
        console.log("trnItiBonafiedCertificateList", result);
  ...
  const onSubmitForm = (formData) => {
    ...
    if (btnSaveText === "Save") {
      ...
      const tempData = axios.post(`${urls.SCHOOL}/trnBonafiedCertifllllicate/save`, _body).then((res) => {
        if (res.status == 201) {
    ...
    // Update Data Based On ID
      ...
      const tempData = axios.post(`${urls.SCHOOL}/trnBonafiedCertiflllicate/save`, _body).then((res) => {
        console.log("res", res);
        if (res.status == 201) {
          formData.id
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.SCHOOL}/trnBonafiedCertificatllllle/save`, body).then((res) => {
      ...
      swal({
        ...
        console.log("inn", willDelete);
        if (willDelete === true) {
          axios.post(`${urls.SCHOOL}/trnBonafiedCertificalllllte/save`, body).then((res) => {
            console.log("delet res", res);
            if (res.status == 201) {
  ...
  return (
    ...
    >
      ...
      <Paper style={{ display: isReady }}>
        {printData && <BonafideToPrint ref={componentRef} data={printData} />}
      </Paper>
      <Box
        sx={{
          display: "flex",
