...
const BachatGatCategory = () => {
    ...
    const deleteById = (value, _activeFlag) => {
        const newState = fetchDocument.map(obj => {
            if (obj.id === value) {
            ...
            renderCell: (record) => {
                return (
                    ...
                    <>
                        ...
                        {record.row.activeFlag == "Y" && <IconButton
                            color="primary"
                        ...
                        {record.row.activeFlag == "N" && <IconButton
                            color="primary"
                            onClick={() => activeById(record.id, "Y")}
                        >
                            <ToggleOffIcon
        ...
        renderCell: (record) => {
            return (
                <>
                    ...
                    {record.row.activeFlag == "N" && <IconButton
                        color="primary"
                        onClick={() => activeMemberById(record.id, "Y")}
                    >
                        <ToggleOffIcon
                            style={{ color: "red", fontSize: 30 }}
                        />
...
const activeMemberById = (value, _activeFlag) => {
    ...
    const handleSearchConnections = (applicationNo) => {
        loggedUser === "citizenUser"
            ? axios
                .get(`${urls.BSUPURL}/trnBachatgatRegistration/getByApplicationNo?applicationNo=${applicationNo}`, {
                    headers: {
                        UserId: user.id
                    }
                },
            ...
            : axios
                .get(
                    `${urls.BSUPURL}/trnBachatgatRegistration/getByApplicationNo?applicationNo=${applicationNo}`,
                    {
                        headers: {
    ...
    const appendUI = () => {
        ...
        if (count <= 20) {
            ...
            alert("Members are less than 20")
        }
    };
    ...
    const handleClose = () => {
        // console.log(fetchDocument[0].srNo)
        // if (attachedFile != null && attachedFile != undefined) {
        setFetchDocuments([
            ...fetchDocument,
            {
                srNo: fetchDocument.length != 0 ? fetchDocument[fetchDocument.length != 0 ? fetchDocument.length - 1 : fetchDocument.length + 1].srNo + 1 : 1,
                id: fetchDocument.length != 0 ? fetchDocument[fetchDocument.length != 0 ? fetchDocument.length - 1 : fetchDocument.length + 1].id + 1 : 1,
                documentKey: null,
    ...
    const onSubmitForm = (formData) => {
        ...
        const finalBodyForApi = {
            ...
            "frontPageFileType": docUpload && docUpload.find((obj) =>
                obj.title == "Passbook Front Page")?.documentPath && docUpload && docUpload.find((obj) =>
                    obj.title == "Passbook Front Page")?.documentPath.split(".").pop(),
            "lastPageFileType": docUpload && docUpload.find((obj) =>
                obj.title == "Passbook Back Page")?.documentPath && docUpload && docUpload.find((obj) =>
        ...
        if (loggedUser === "citizenUser") {
            const tempData = axios
                .post(`${urls.BSUPURL}/trnBachatgatRegistration/modifyBachatGat`, finalBodyForApi, {
                    headers: {
                        UserId: user.id,
                    },
                })
                .then((res) => {
                    if (res.status == 201) {
                        ...
                        sweetAlert({
                            text: ` Your Bachatgat Registration Application No Is : ${res.data.message.split('[')[1].split(']')[0]} Modified Successfully`,
                            icon: "success",
                            ...
                            if (will) {
                                ...
                                router.push({
                                    ...
                                    query: { id: res.data.message.split('[')[1].split(']')[0], trn: "M" },
                                })
                            }
                        })
                    }
                ...
                <Paper elevation={8}
                    ...
                    <form onSubmit={handleSubmit(onSubmitForm)}>
                        <Grid container style={{ padding: "10px" }}>
                            ...
                            {/* add more btn */}
                            <Grid container>
                                {!isOpenCollapse && <Grid item xs={12} style={{ display: "flex", justifyContent: "end", margin: "5px" }}>
                                    <Button
                                        variant="contained"
                                        size="small"
                                        startIcon={<AddIcon />}
