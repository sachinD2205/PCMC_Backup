...
const BachatGatCategory = () => {
  ...
  const setToDataTable = (data, _pageSize, _pageNo) => {
    ...
    if (result != null  && result.length != 0 ) {
      if (wardNames && zoneNames && crAreaNames) {
        ...
        let filteredArrayZone = _res?.filter((obj) => obj?.trnType === "BGR");
  ...
  const columns = [
    ...
    {
      field: "createdDate",
      headerName: <FormattedLabel id="RegistrationDate" />,
      width: 200,
      align: "left",
      headerAlign: "center",
    },
          ...
          <>
            ...
            <IconButton
              onClick={() => {
                ((loggedUser == "citizenUser" || loggedUser === "cfcUser") && params.row.statusVal == 1) ? router.push({
                  ...
                  router.push({
                    pathname: "/BsupNagarvasthi/transaction/bachatgatRegistration/view",
                    query: {
                      id: params.row.id,
                    },
                  });
            ...
            {((loggedUser == "citizenUser" || (loggedUser === "cfcUser")) && params.row.statusVal === 2) && (<IconButton
              onClick={() => {
                router.push({
                  ...
                  query: {  id: params.row.applicationNo, trn: "R" },
                })
              }}
            >
            ...
            {((loggedUser == "citizenUser" || loggedUser === "cfcUser") && (params.row.statusVal === 10)) ?
              <IconButton
                onClick={() => {
                  router.push({
                    pathname: "/BsupNagarvasthi/transaction/bachatgatRenewal/form",
                    query: {
                      id: params.row.id,
                    },
                  })
              ...
              >
                <Tooltip title={`Renewation`}>
                  <AutorenewIcon style={{ color: "#556CD6" }} />
                </Tooltip>
              </IconButton> : <></>}
          </>
        );
  ...
  return (
    <div>
      <Paper elevation={8}
        ...
        >
          <h2>
            <FormattedLabel id="bachatgatRegistration" />
          </h2>
        </Box>
