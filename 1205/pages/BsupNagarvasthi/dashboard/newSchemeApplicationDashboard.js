...
const Index = () => {
    ...
    useEffect(() => {
        ...
        setMainSchemeFromDate(filtered[0].value.split("-")[0] + "-04-01" + " 00:00:00")
        setMainSchemeToDate(filtered[0].value.split("-")[1] + "-03-31" + " 23:59:59")
        setValue("mainSchemeYearKey", 1)

        setSubSchemeFromDate(filtered[0].value.split("-")[0] + "-04-01" + " 00:00:00")
        setSubSchemeToDate(filtered[0].value.split("-")[1] + "-03-31" + " 23:59:59")
        setValue("subSchemeYearKey", 1)

        setMainSubSchemeFromDate(filtered[0].value.split("-")[0] + "-04-01" + " 00:00:00")
        setMainSubSchemeToDate(filtered[0].value.split("-")[1] + "-03-31" + " 23:59:59")
        setValue("mainsubSchemeYearKey", 1)
    }, [])


    const getApplicationSchemeCount = () => {
        axios.get(`${urls.BSUPURL}/dashboard/getNewSchemeApplicationCounts`).then((r) => {
    ...
    const getSubSchemeDetails = () => {
        if (subSchemeFromDate && subSchemeToDate) {
            ...
            axios.post(`${urls.BSUPURL}/dashboard/getAllSubSchemeWiseSummary`, body).then((r) => {
                const result = r.data
    ...
    const getMainAndSubSchemeDetails = () => {
        if (mainSubSchemeToDate && mainSubSchemeFromDate) {
            ...
            axios.post(`${urls.BSUPURL}/dashboard/getAllZoneMainSchemeAndSubSchemeWiseSummary`, body).then((r) => {
                const result = r.data
                let _res = result?.map((r, i) => {
                    return {
                        srNo: i + 1,
    ...
    const callMainSchemeList = (e, index) => {
        if (index === 1) {
            setMainSchemeFromDate(yearWiseArray[e - 1].value.split("-")[0] + "-04-01" + " 00:00:00")
            setMainSchemeToDate(yearWiseArray[e - 1].value.split("-")[1] + "-03-31" + " 23:59:59")
        } else if (index == 2) {
            setSubSchemeFromDate(yearWiseArray[e - 1].value.split("-")[0] + "-04-01" + " 00:00:00")
            setSubSchemeToDate(yearWiseArray[e - 1].value.split("-")[1] + "-03-31" + " 23:59:59")
        } else {
            setMainSubSchemeFromDate(yearWiseArray[e - 1].value.split("-")[0] + "-04-01" + " 00:00:00")
            setMainSubSchemeToDate(yearWiseArray[e - 1].value.split("-")[1] + "-03-31" + " 23:59:59")
        }
    }
    ...
    const getMainSchemeDetails = () => {
        if (mainSchemeFromDate && mainSchemeToDate) {
            ...
            axios.post(`${urls.BSUPURL}/dashboard/getAllMainSchemeWiseSummary`, body).then((r) => {
                const result = r.data
                let _res = result?.map((r, i) => {
                    return {
                        srNo: i + 1,
    ...
    return (
        <>
            <Paper elevation={8}
                        ...
                        <Grid container style={{ display: "flex", justifyContent: "center" }}>
                            {[
                                ...
                                },
                                {
                                    ...
                                    icon: "Menu",
                                    count: totalCount,
                                    name: <FormattedLabel id="totalCount" />,
                                    bg: "#FFC0CB",
                                },
                                {
                                    id: 5,
                ...
                >
                    <h2> <FormattedLabel id='mainSchemeSummary' /></h2>
                </Box>

                <Grid item xs={4}>
                    <FormControl
                        sx={{ width: "280px", margin: "2%" }}
                        variant="standard"
                    >
                ...
                >
                    <h2> <FormattedLabel id='subSchemeSummary' /></h2>
                </Box>
                <Grid item xs={4}>
                    <FormControl
                        sx={{ width: "280px", margin: "2%" }}
                ...
                >
                    <h2> <FormattedLabel id='mainSubSchemeSummary' /></h2>
                </Box>
                <Grid item xs={4}>
                    <FormControl
                        sx={{ width: "280px", margin: "2%" }}
