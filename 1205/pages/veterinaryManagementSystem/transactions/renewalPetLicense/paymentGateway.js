...
const Index = () => {
  ...
  const [paymentType, setPaymentType] = useState([
    { type: "online", nameEn: "Net Banking", nameMr: "Net Banking in marathi", value: "Net Banking" },
    { type: "online", nameEn: "UPI", nameMr: "UPI in marathi", value: "UPI" },
    { type: "offline", nameEn: "Demand Draft", nameMr: "Demand Draft in marathi", value: "Demand Draft" },
    { type: "offline", nameEn: "Cash", nameMr: "Cash in marathi", value: "Cash" },
  ]);

  let petSchema = yup.object().shape({
    receivedFrom: yup.string().required("Please enter a name"),
    amount: yup.string().required("Please enter an amount"),
    paymentMode: yup.string().required("Please select a mode"),
  ...
  const finalSubmit = (data) => {
    ...
    if (watch("paymentMode") === "online") {
      if (watch("paymentType") === "Net Banking") {
        const { bankName, branchName, ifscCode, accountNo } = data;
    ...
    axios.post(`${URLs.VMS}/trnRenewalPetLicence/processPaymentCollection`, bodyForAPI).then((res) => {
      if (res.status == 201 || res.status == 200)
        sweetAlert({
          ...
          if (ok) {
            router.push({
              pathname: `/veterinaryManagementSystem/transactions/renewalPetLicense/paymentSlip`,
    ...
    <>
      ...
      <Paper className={styles.main}>
        ...
        <form onSubmit={handleSubmit(finalSubmit)} style={{ padding: "3vh 3%" }}>
          ...
          {watch("paymentMode") === "online" && watch("paymentType") === "Net Banking" && (
            <div className={styles.row}>
          ...
          {watch("paymentMode") === "offline" && watch("paymentType") === "Demand Draft" && (
            <div className={styles.row}>
              <TextField
                sx={{ width: "250px" }}
          ...
          <div className={styles.buttons}>
            ...
            <Button
              ...
              onClick={() => {
                ...
                isDeptUser ? router.back() : router.push("/dashboard");
              }}
            >
