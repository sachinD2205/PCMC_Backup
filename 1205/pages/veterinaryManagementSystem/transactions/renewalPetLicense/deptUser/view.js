    ...
    if (router.query.id) {
      ...
      axios
        ...
        .then((res) => {
          ...
          reset({
            action: res.data.status == "Reassigned by HOD" ? "Reassigned by Clerk" : "Approved by Clerk",
        ...
        axios
          ...
          .then((res) => {
            if (res.status === 200 || res.status === 201) {
              if (watch("action") === "Approved by Clerk") {
                sweetAlert("Success!", "Application approved!", "success");
              } else {
                sweetAlert("Success!", "Application reassigned!", "success");
              }

              router.push(`/veterinaryManagementSystem/transactions/renewalPetLicense/deptUser`);
            }
          })
          .catch((error) => {
            console.log("error: ", error);
    ...
    <>
      ...
      <Paper className={styles.main}>
        ...
        <form onSubmit={handleSubmit(finalSubmit)} style={{ padding: "5vh 3%" }}>
          ...
          {(applicationDetails.status == "Applied" || applicationDetails.status == "Reassigned by HOD") && (
            <div
                ...
                <Controller
                  render={({ field }) => (
                    <Select
                      ...
                      label="action"
                    >
                      <MenuItem key={1} value={"Approved by Clerk"}>
                        {language === "en" ? "Approve" : "मंजूर"}
                      </MenuItem>
                      <MenuItem key={2} value={"Reassigned by Clerk"}>
          ...
          <div className={styles.buttons}>
            {
              ...
              (applicationDetails.status == "Applied" ||
                // @ts-ignore
                applicationDetails.status == "Reassigned by HOD") && (
                <Button
            ...
            <Button
              ...
              onClick={() => {
                router.push({
                  pathname: `/veterinaryManagementSystem/transactions/renewalPetLicense/deptUser`,
                });
              }}
            >
