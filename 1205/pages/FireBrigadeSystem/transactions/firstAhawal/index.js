...
const Index = () => {
  ...
  const getVardiTypes = () => {
    axios.get(`${urls.BaseURL}/vardiTypeMaster/getVardiTypeMasterData`).then((r) => {
      let vardi = {};
      r.vardiSlip?.data.map((r) => (vardi[r.vardiSlip.id] = r.vardiSlip.vardiName));
      setVardiTypes(vardi);
    });
  };
          ...
          <Box>
            <IconButton
              ...
              onClick={() => {
                ...
                router.push(
                  {
                    pathname: "/FireBrigadeSystem/transactions/firstAhawal/form",
                    query: {
                      pageMode: "Edit",
                      ...record,
    ...
    <Box sx={{ width: "100%", height: "100%" }}>
      <Box style={{ display: "flex" }}>
        <Box className={styles.tableHead}>
          <Box className={styles.h1Tag}>{<FormattedLabel id="firstVardi" />}</Box>
        </Box>
      </Box>
