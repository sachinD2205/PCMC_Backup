...
import { FormControl, FormHelperText, Paper, TextField, Button, Checkbox } from "@mui/material";
  ...
  const Comp = () => {
    return (
      <div className={styles.reportWrapper} ref={componentRef}>
        ...
        <span>
          <strong>{"विषय क्र. १) मा. शहर सुधारणा समितीच्या सभापतींची नेमणूक करणे"}</strong>
        </span>
        <p className={styles.description}>
          {
          ...
          <>
            <IconButton
              onClick={() => {
                ...
                router.push({
                  pathname: "/municipalSecretariatManagement/reports/meetingAgenda/print",
        ...
        if (res.data.prepareMeetingAgenda.length > 0) {
          setTable(
            res.data.prepareMeetingAgenda.map((j, i) => ({
              ...
              committeeNameEn: committeeName?.find((obj) => obj.id === j.committeeId)?.committeeNameEn,
              committeeNameMr: committeeName?.find((obj) => obj.id === j.committeeId)?.committeeNameMr,
    ...
    <>
      ...
      <Paper className={styles.main}>
        <form onSubmit={handleSubmit(submit)}>
          ...
          <div className={styles.row} style={{ justifyContent: "center", columnGap: 100 }}>
            {checkBoxState ? (
              <TextField
