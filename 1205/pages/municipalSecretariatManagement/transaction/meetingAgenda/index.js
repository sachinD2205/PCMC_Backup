...
const Index = () => {
  ...
  useEffect(() => {
    if (data.rows.length > 0) {
      ...
      data.rows?.map((obj) => {
        let committeeName1 = comittees1?.find((ob) => ob.id === obj.committeeId)?.comittee;
        setValue("agendaDescription", obj.agendaDescription);
        //   setValue("agendaNo", obj.agendaNo)
        setValue("agendaOutwardDate", moment(obj.agendaOutwardDate).format("YYYY-MM-DD"));
              ...
              >
                ...
                >
                  <TextField
                    ...
                    helperText={errors?.karyakramPatrikaNo ? errors.karyakramPatrikaNo.message : null}
                  />
                </Grid>
