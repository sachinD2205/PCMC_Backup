...
const Index = () => {
  ...
  const [gettingIdsInResponse, setGettingIdsInResponse] = useState([]);
  // @ts-ignore
  ...
  const editById = (values) => {
    ...
    // let responseIdsArra = values?.committeeId?.split(",").slice(1, -1).map(Number);
    // console.log(":lok2..responseIdsArra", responseIdsArra);

    // let macthingObj = committeeName?.filter((obj) => responseIdsArra?.includes(obj.id));
    // console.log(":lok..macthingObj", macthingObj);
    // setSelectedValues(
    //   macthingObj?.map((obj) => (language === "en" ? obj.committeeNameEn : obj.committeeNameMr)),
    // );

    setSelectedValues(values?.committeeId?.split(",").slice(1, -1).map(Number));
    setCollapse(true);
  };
              ...
              <form onSubmit={handleSubmit(finalSubmit)}>
                <div className={styles.main} style={{ marginTop: 3 }}>
                  <div className={styles.row}>
                    ...
                    {/* ///////////////////////////// REMOVING AFTER DISCUSSION WITH AMOL SIR
                    ///////////////////////////// */}
              ...
              return ob.id === selecedHOD
                  ...
                  <div className={styles.row}>
                    ...
                    <TextField
                      ...
                      error={!!error.outwardNumber}
                      helperText={error?.outwardNumber ? error.outwardNumber.message : null}
                    />
                  </div>
