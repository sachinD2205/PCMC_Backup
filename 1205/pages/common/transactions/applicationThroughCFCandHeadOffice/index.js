...
const Index = () => {
  ...
  // define form 
  const [applicationThroughCFCandHeadOfficeViewEdit] = Form.useForm()

  // Get Api 
  const getApplicationThroughCFCandHeadOffice = () => {
    axios.get(`${urls.BaseURL}/ApplicationThroughCFCAndHeadOffice/getApplicationThroughCFCAndHeadOfficeDetails`).then(
      (resp) => {
        setDataSource(
          resp.data.map(
            (r, i) => ({
  ...
  // useEffect
  useEffect(() => {
    getApplicationThroughCFCandHeadOffice()
  }, [])
  ...
  const showModalForEdit = (record) => {
    applicationThroughCFCandHeadOfficeViewEdit.resetFields()
    setDataInModal(
      {
        id: record.id,
        applicationDate: moment(record.applicationDate, 'YYYY-MM-DD').format('YYYY-MM-DD'),
  ...
  const editForm = async () => {
    ...
    if (okText == 'Save') {
      await axios.put(`
        ${urls.BaseURL}/ApplicationThroughCFCAndHeadOffice/updateApplicationThroughCFCAndHeadOffice
        `, parametersForEdit).then(
        (resp) => {
          if (resp.status == 200) {
            message.success('Data Updated !!!')
            setIsModalVisible(false)
            router.push(`/common/transactions/applicationThroughCFCandHeadOffice/`)
          }
        }
      )
    }
  ...
  const deleteForm = async (record) => {
    ...
    await axios.delete(`${urls.BaseURL}/ApplicationThroughCFCAndHeadOffice/deleteApplicationThroughCFCAndHeadOffice/${recordId}`).then(
      (resp) => {
        if (resp.status == 200) {
  ...
  return (
    <div>
      <BasicLayout titleProp={'Application Through CFC and Head Office'}>
        <Card>
          <div style={{ display: 'flex', justifyContent: 'right' }}>
            <Button
              ...
              onClick={() => {
                router.push(
                  `/common/transactions/applicationThroughCFCandHeadOffice/view`,
                )
              }}
            >
              {' '}
          ...
          >
            ...
            <Form
              title='applicationThroughCFCandHeadOffice'
              form={applicationThroughCFCandHeadOfficeViewEdit}
              layout='vertical'
            >
