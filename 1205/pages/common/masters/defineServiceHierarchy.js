...
const Index = () => {
  ...
  // Get Table - Data
  const getServiceMaster = (_pageSize = 10, _pageNo = 0, _sortBy = "id", _sortDir = "desc") => {
    setOpen(true);
    axios
      ...
      .then((res) => {
        if (res.status == 200) {
          ...
          let _res = result.map((r, i) => {
            console.log("res payment mode", departments?.find((obj) => obj?.id == r.department)?.department);
            return {
              ...
              serviceNameFeild: serviceList?.find((obj) => obj?.id == r.service)?.serviceName,
              ...
              roleColEn: roleList.filter((f) => f.id == role)?.name,
              roleColMr: roleList.find((f) => f.id == role)?.nameMr,
            };
            setOpen(false);
          });
  ...
  const onSubmitForm = (formData) => {
    ...
    axios.post(`${urls.BaseURL}/serviceHierarchy/save`, finalBodyForApi).then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.BaseURL}/serviceHierarchy/save`, body).then((res) => {
                            ...
                            >
                              <Grid item md={4}>
                                <TextField
                                  ...
                                  {...register(`levelsOfRolesDaoList.${index}.level`)}
                                  error={errors?.levelsOfRolesDaoList?.[index]?.level}
                                  helperText={
                                    errors?.levelsOfRolesDaoList?.[index]?.level
                                      ? errors.levelsOfRolesDaoList?.[index]?.level.message
                                      : null
                                  }
                                />
                              </Grid>
                              <Grid item md={4}>
                                <FormControl style={{ width: "100%" }} size="small">
                                  ...
                                  <Controller
                                    render={({ field }) => (
                                      <Select
                                        error={errors?.levelsOfRolesDaoList?.[index]?.role ? true : false}
                                        labelId="demo-simple-select-label"
                                        id="demo-simple-select"
                                  ...
                                  <FormHelperText style={{ color: "red" }}>
                                    {errors?.levelsOfRolesDaoList?.[index]?.role
                                      ? errors?.levelsOfRolesDaoList?.[index]?.role.message
                                      : null}
                                  </FormHelperText>
                                </FormControl>
                              </Grid>
