...
const NewIndex = () => {
  ...
  // get all data
  const getApprovalRejectionRemarks = (_pageSize = 10, _pageNo = 0, _sortBy = "id", _sortDir = "desc") => {
    setLoading(true);
    axios
      ...
      .then((res) => {
        ...
        let _res = result.map((j, i) => {
          ...
          return {
            ...
            applicationCol: applications.find((f) => f.id == j.applicationId)?.applicationNameEng,

            departmentId: j.departmentId,
            departmentCol: departmentDropDown.find((f) => f.id == j.departmentId)?.department,

            serviceId: j.serviceId,

            serviceCol: serviceDropDown.find((f) => f.id == j.serviceId)?.serviceName,
  ...
  const editById = (values) => {
    ...
    reset({ ...values, departmentId: deptId, serviceId: serviceId, applicationId: appId });
    setCollapse(true);
  };
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.CFCURL}/master/approvalRejectionRemarks/save`, body).then((res) => {
                      ...
                      <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        >
                          <TextField
                            ...
                            disabled={isDisabled}
                            defaultValue={router.query.remark ? router.query.remark : ""}
                          />
                          <TextField
                            sx={{
                              width: "450px",
                            ...
                            disabled={isDisabled}
                            defaultValue={router.query.remarkMr ? router.query.remarkMr : ""}
                          />
                        </div>
