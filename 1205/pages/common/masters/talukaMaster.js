...
const Index = () => {
 ...
 ;
  ...
  const gettypeOfRemarks = () => {
    axios.get(`${urls.BaseURL}/department/getAll`).then((r) => {
      settypeOfRemarks(
        // r.data.map((row) => ({
        //   id: row.id,
  ...
  // Get Table - Data
  const getTalukaMaster = () => {
    axios.get(`${urls.BaseURL}/talukaMaster/getAll`).then((res) => {
      console.log(` ----- ${res.data}`);
      setDataSource(
        res.data.districtMaster.map((r, i) => ({
          id: r.id,
          srNo: i + 1,
          talukaNameEn: r.talukaNameEn,
          talukaNameMr: r.talukaNameMr,
  ...
  const onSubmitForm = (formData) => {
    ...
    axios
      .post(`${urls.BaseURL}/talukaMaster/save`, finalBodyForApi)
      .then((res) => {
        if (res.status == 200) {
          formData.id
...
//   };
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.CFCURL}/master/talukaMaster/save`, body)
            .then((res) => {
  ...
  const columns = [
    ...
    {
      field: "talukaNameEn",
      headerName: "Taluka Name(English)",
      // type: "number",
      flex: 1,
    },
    {
      field: "talukaNameMr",
      headerName: "Taluka Name(Marathi)",
      //type: "number",
      flex: 1,
    },
    // {
              ...
              <div>
                <FormProvider {...methods}>
                  <form onSubmit={handleSubmit(onSubmitForm)}>
                    <div>
                      <Grid container spacing={2} style={{marginTop:"5px",marginLeft:"5%"}}>
                        <Grid item xs={4}>
                          <TextField
                            sx={{ width: 250, marginTop: 5 }}
                            id="standard-basic"
                            label="Taluka Name Eng*"
                            variant="standard"
                            {...register("talukaNameEn")}
                            error={!!errors.talukaNameEn}
                            helperText={
                              errors?.talukaNameEn ? errors.talukaNameEn.message : null
                            }
                          />
                        </Grid>
                        <Grid item xs={4}>
                          <TextField
                            sx={{ width: 250, marginTop: 5 }}
                            id="standard-basic"
                            label="Taluka Name Mr"
                            variant="standard"
                            {...register("talukaNameMr")}
                            error={!!errors.talukaNameMr}
                            helperText={
                              errors?.talukaNameMr ? errors.talukaNameMr.message : null
                            }
                          />
                        </Grid>
                ...
                >
                  ...
                  {/* </FormControl>
                      ...
                      <Grid container spacing={2} style={{marginTop:"5%",marginLeft:"20%"}}>
