...
function AuditoriumOtherCharges() {
  ...
  const onSubmitFunc = (data) => {
    ...
    if (watch("fromDate") && watch("toDate")) {
      ...
      setLoading(true);
      axios
        .get(`${urls.PABBMURL}/extraEquipmentUsedCharges/getExtraEquipmentUsedReport`, {
          params: {
            fromDate: fromDate,
            toDate: toDate,
        ...
        .then((res) => {
          ...
          if (res?.status === 200 || res?.status === 201) {
            if (res?.data?.extraEquipmentUsedChargesList?.length > 0) {
              let _res = res?.data?.extraEquipmentUsedChargesList?.map((r, i) => {
    ...
    <Paper>
      ...
      >
        ...
        <Grid item xs={8} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
          <h2>{language === "en" ? "Auditorium Other Charges" : "सभागृह इतर शुल्क"}</h2>
        </Grid>
        {/* <Grid item xs={2} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
            <Button
              size="small"
