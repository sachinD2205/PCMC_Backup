...
// import styles from '../partMap/index.module.css'
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${URLS.TPURL}/zoneCertificate/getZoneCertificateDetails`)
      .then((response) => {
  ...
  const deleteRecord = async (record) => {
    sweetAlert({
      ...
      if (willDelete) {
        axios
          .delete(
            `${URLS.TPURL}/zoneCertificate/deleteZoneCertificate/${record.id}`
          )
          .then((res) => {
            if (res.status == 200) {
    ...
    <>
      ...
      <div>
        <Button
          ...
          onClick={() =>
            router.push({
              pathname: `/townPlanning/transactions/zoneCertificate/view`,
              query: {
                pageMode: 'new',
