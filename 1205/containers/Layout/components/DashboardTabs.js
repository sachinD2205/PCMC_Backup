...
import { Box, Typography, Grid, Tooltip, Card, CardHeader, Tabs, Badge } from "@mui/material";
...
const DashboardTabs = (props) => {
  ...
  const usersCitizenDashboardData = useSelector((state) => {
    console.log("usersCitizenDashboardData", state.user.usersCitizenDashboardData);
  ...
  function getServicesByAppId(appId) {
    return usersCitizenDashboardData?.services.filter((v) => appId === v.application).map((r) => r);
  }

  console.log("222", services);
                    ...
                    >
                      ...
                      >
                        <Box style={{ height: "55%" }}>
                          ...
                          >
                            {language === "en" ? tab.applicationNameEng : tab.applicationNameMr}
          ...
          {services?.length > 0 ? (
            services.map((tab, id) => {
              return (
                ...
                >
                  <Tooltip title={language === "en" ? tab.serviceName : tab.serviceNameMr}>
                    <Card
                      sx={{ width: "80%", height: "90%", cursor: "pointer" }}
                      onClick={() => {
                        console.log("tab", tab);
                        dispatch(setApplicationName(tab));
                        user.isProfileComleted === "Y"
                          ? tab.clickTo && tab.application == 6
                            ? router.push({ pathname: tab.clickTo, query: { pageMode: "Add" } })
                    ...
                    >
                      <CardHeader
                        ...
                        title={language === "en" ? tab.serviceName : tab.serviceNameMr}
                      />
                    </Card>
                  </Tooltip>
                </Grid>
            ...
            <Box>
              <Card>
                <CardHeader
                  ...
                  title={language === "en" ? "Services Not Available" : "सेवा उपलब्ध नाही"}
                />
              </Card>
            </Box>
          )}
