...
import { Box, Grid, Button, IconButton, Modal, Typography } from "@mui/material";
    ...
    props.newFilesFn((oldElements) => {
      ...
      return [
        ...oldElements,
        {
          ...
          attachedNameEn:
            user?.userDao?.firstNameEn + " " + user?.userDao?.middleNameEn + " " + user?.userDao?.lastNameEn,
          attachedNameMr:
            user?.userDao?.firstNameMr + " " + user?.userDao?.middleNameMr + " " + user?.userDao?.lastNameMr,
        },
      ];
    });
    props.filePath("");
      ...
      {props.authorizedToUpload ? (
        <div className={styles.mainButton}>
          ...
          >
            ...
            {/* ADD DOCUMENTS */}
            <Button  onClick={handleOpen} endIcon={<Add sx={{ color: "white" }} />} sx={{ color: "white" }}>
