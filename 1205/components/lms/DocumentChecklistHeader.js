...
const Index = () => {
  ...
  useEffect(() => {
    ...
    if (router?.query) {
      setData(router?.query)
      reset(router?.query)
      setValue('applicationDate', moment(router?.query?.applicationDate).format("DD-MM-YYYY"))
    ...
    if (ID) {
      ...
      axios
        ...
        .then((resp) => {
          ...
          setData(resp.data)
          setValue('applicationDate', moment(resp.data.applicationDate).format("DD-MM-YYYY"))
        })
    }
  }, [])
