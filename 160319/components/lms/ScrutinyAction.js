...
const Index = (props) => {
    ...
    const remarks = async (props) => {
        ...
        console.log('a12345678', router?.query?.id, watch('remark'))
        if (router?.query?.id) {
            let finalbody = {
                id: router?.query?.id,
                isApprove: true,
        ...
        if (serviceId == 84) {
            ...
            // axios
            //     .post(
            //         `${urls.LMSURL}/trnApplyForNewMembership/updateStatus/saveMarraigeBoardRegistrationApprove?id=${ID}&isApprove=${true}&remark=${}`,
            //     )
            //     .then((response) => {
            //         if (response.status === 200) {
            //             router.push(
