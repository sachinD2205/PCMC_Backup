...
import style from "../../../styles/skysignstyles/upload.module.css";
...
const UploadButton = ({ Change }) => {
  ...
  // discardFile = () => {
  //   axios.post(`http://localhost:8094/lc/api/notice/discard?${file.name}`)
  // }
    ...
    <div className={style.align}>
      <label className={style.uploadButton}>
        ...
        {file ? (
          ...
          <span className={style.fileName}>{<FormattedLabel id="attachFile"></FormattedLabel>}</span>
        )}
      </label>
      {file && (
        <IconButton
