...
import styles from "../../../../components/streetVendorManagementSystem/styles/licenseValidity.module.css";
...
const Index = () => {
  ...
  const getLicenseValidityDetails = () => {
    axios.get(`${urls.HMSURL}/licenseValidity/getAll`).then((res) => {
      console.log(res);
      setDataSource(
        res.data.licenseValidity.map((r, i) => ({
          id: r.id,
          srNo: i + 1,
  ...
  const deleteById = (value) => {
    swal({
      ...
      if (willDelete) {
        axios
          .delete(
            `${urls.HMSURL}/licenseValidity/discardLicenseValidity/${value}`,
          )
          .then((res) => {
            if (res.status == 226) {
