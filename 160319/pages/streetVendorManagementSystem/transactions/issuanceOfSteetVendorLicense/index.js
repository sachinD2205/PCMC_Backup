...
function getSteps(i) {
  return [
    ...
    <strong key={5}>{<FormattedLabel id="additionalDetails" key={5} />}</strong>,
...
const LinaerStepper = () => {
  ...
  const { register, getValues, setValue, handleSubmit, methos, watch } = methods;
        ...
        .then((res) => {
          ...
          if (res?.status == 200 || res?.status == 201 || res?.status == "SUCCESS") {
            ...
            res?.data?.id
              ...
              : sweetAlert("Submitted !", res?.data?.message, "success");
            // res?.data?.id
            //   ? sweetAlert("Submitted!", "your application number is : PCMC9032023HMS000000000002", "success")
            //   : sweetAlert(
            //       "Submitted !",
            //       "your application number is : PCMC9032023HMS000000000002",
            //       "success",
            //     );
