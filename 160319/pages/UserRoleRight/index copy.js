...
const UserRoleRight = () => {
  ...
  const isAllSelected =
    roleList.length > 0 && selected.length === roleList.length
  ...
  const getApplicationsName = async (value) => {
    await axios
      .get(`${urls.CFCURL}/master/application/getByUserId?userId=${value}`)
      .then((r) => {
        if (r.status == 200) {
  ...
  const onFinish = async (data) => {
    ...
    // await axios
    //   .post("http://localhost:8090/cfc/api/master/userRoleMenu/save", body)
    //   .then((r) => {
    //     if (r.status == 200) {
    //       console.log("res", r);
    //     } else {
                ...
                >
                  {designationList.length > 0
                    ? designationList
                        .filter(
                          (designation) =>
                            designation.departmentId === selectedDepartment
                        )
