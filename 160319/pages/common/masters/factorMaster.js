...
const FactorMaster = () => {
  ...
  // Get Table - Data
  const getcast = () => {
    axios.get(`${urls.CFCURL}/master/factorMaster/getAll`).then((res) => {
  
      setDataSource(
        res.data.factor.map((r, i) => ({
          id: r.id,
          srNo: i + 1,
          ...
          // application: application?.find((obj) => obj?.id === r.applicationId)
          //   ?.applicationId,

          applicationId: application[r.applicationId]
            &&  application[r.applicationId].applicationId,
          // cast: r.cast,
          remark: r.remark,
                          ...
                          >
                            ...
                            <Controller
                              render={({ field }) => (
                                ...
                                >
                                  {application &&
                                    application.map((applicationId, index) => (
                                      <MenuItem key={index} value={applicationId.id}>
