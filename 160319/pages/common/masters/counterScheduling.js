...
const NewIndex = () => {
  let schema = yup.object().shape({
    ...
    applicationCountPerEmployeeForADay: yup
      .string()
      .required("Please select a applicationCountNo."),
  ...
  const getCounterSchedulind =() => {
    ...
    axios.get(`${urls.CFCURL}/master/counterScheduling/getAll`).then((r) => {
      console.log("Remark data:", r.data);
      setTable(
        r.data.counterSheduling.map((j, i) => ({
          id: j.id,
          srNo: i + 1,
          ...
          zoneName: zoneDropDown[j.zoneId]
          ? zoneDropDown[j.zoneId].zoneName
          : "-",
          serviceName : serviceDropDown[j.serviceId]
          ? serviceDropDown[j.serviceId].serviceName
          : "-",
          wardName : wardDropDown[j.wardId]
          ? wardDropDown[j.wardId].wardName
          : "-",
  ...
  const onSubmit = (formData) => {
    ...
    axios
      .post(
        `${urls.CFCURL}/master/counterScheduling/save`,
        finalBodyForApi
      )
      .then((res) => {
