...
const View = () => {
  ...
  const finalSubmit = async () => {
    ...
    //   await axios
    //     .post(`${URLS.BaseURL}/partplan/updatepartplan`, bodyForAPI, {
    //       headers: {
    ...
    // } else {
    //   await axios
    //     .post(`${URLS.BaseURL}/partplan/savepartplan`, bodyForAPI, {
    //       headers: {
    //         // Authorization: `Bearer ${token}`,
    //         role: 'CITIZEN',
    //       },
  ...
  const uploadAgain = async () => {
    ...
    await axios
      ...
      .then((response) => {
        if (response.status === 200) {
          sweetAlert('Saved!', 'Record Saved successfully !', 'success')
          router.push('/townPlanning/transactions/generationofTDRFSI')
        }
      })
  }
  ...
  return (
    <>
      <Head>
        <title>Generation of TDR/FSI</title>
      </Head>
