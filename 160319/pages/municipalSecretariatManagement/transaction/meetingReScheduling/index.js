  ...
  const getAllAmenities = (_pageSize = 10, _pageNo = 0) => {
    axios
      ...
      .then((res) => {
        ...
        setValue("meetingDate", result[0]?.previousMeetingDate)
        setValue("agendaNo", queryParams?.agendaNo)
        setValue("meetingPlace", result[0]?.previousMeetingPlace)
        ...
        axios
          ...
          .then((res) => {
            if (res.status == 200 || res.status == 201) {
              sweetAlert(
                "Saved!",
                "MeetingRescheduled  successfully !",
                "success"
              ).then((will) => {
                if (will) {
                  reset({
                  ...
                  filterDocketRemoveToLocalStorage("startDate")
