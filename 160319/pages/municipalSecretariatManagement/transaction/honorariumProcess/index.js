...
const Index = () => {
  ...
  let honorariumSchema = yup.object().shape({
    ...
    deductedOtherAmount: yup
      .number()
      .required('Please enter some (if any) deducted amount'),
    healthInsuranceCharges: yup
      .number()
      .required('Please enter health insurance charges'),
  })
  ...
  const getAttendance = () => {
    let selectedMonth = watch('month')
    let selectedYear = watch('year')
        ...
        <form className={styles.main} onSubmit={handleSubmit(finalSubmit)}>
          ...
          <div className={styles.row}>
            ...
            <FormControl variant="standard" error={!!error.wardNo}>
              ...
              />
              <FormHelperText>
                {error?.wardNo ? error.wardNo.message : null}
              </FormHelperText>
            </FormControl>
