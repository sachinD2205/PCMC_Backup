...
const Index = () => {
  ...
  const [marksPrintData, setMarksPrintData] = useState();
  ...
  useEffect(() => {
    const getExamList = async () => {
      ...
      try {
        const { data } = await axios.get(
          `${urls.SCHOOL}/mstExam/getExamListForDivision?schoolKey=${schoolId}&academicYearKey=${academicYearId}&classKey=${classId}&divisionKey=${divisionId}`,
        );
  ...
  const findMarks = async () => {
    ...
    try {
      ...
      const subjectMarks = subRes.data.mstAcademicSubjectList.map((sub) => ({
        id: nanoid(),
        subjectKey: sub.id,
  ...
  const handleMarksPrintData = async () => {
    ...
    try {
      ...
      setMarksPrintData(studentMarksData);
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  const handleStudentPrintData = async () => {
    ...
    try {
      await axios.get(
        `${urls.SCHOOL}/mstStudent/getById?id=${student.id}`,
      ).then((r) => {
    ...
    >
      <Grid container gap={4} direction="column">
        ...
        <Paper style={{ display: isReady }}>
          {(marksPrintData && studentPrintData) && <StudentsMarksReportToPrint ref={componentRef} data={marksPrintData} stuData={studentPrintData} />}
        </Paper>
        <form>
            ...
            >
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  // handlePrint();
                  setIsReady("none");
                  console.log("marksPrintData", marksPrintData)
