...
import schema from "../../../../containers/schema/LegalCaseSchema/responseToNoticeSchema";
...
const ParawiseReport = () => {
  ...
  } = useFieldArray({ control, name: "responseToNoticeDaoLst" });
  const {
    fields: noticeFields,
    append: noticeAppend,
    remove: noticeRemove,
  ...
  useEffect(() => {
    ...
    selectedNotice.parawiseTrnParawiseReportDaoLst?.map((val, index) => {
      ...
      return ParawiseAppend({
        parawiseReportId: val?.id,
        issueNo: index + 1,
        paragraphWiseAanswerDraftOfIssuesOg:
  ...
  useEffect(() => {
    ...
    const RTN = [...selectedNotice.parawiseTrnParawiseReportDaoLst];
  ...
  useEffect(() => {
    console.log("Table Files: ", additionalFiles, mainFiles);
  ...
  //     issueNo: "",
  //     paragraphWiseAanswerDraftOfIssues:
  //       "selectedNotice.parawiseTrnParawiseReportDaoLst?.paragraphWiseAanswerDraftOfIssues",
  //   });
  // }, [selectedNotice.parawiseTrnParawiseReportDaoLst]);
  ...
  //     headerName: "Paragraph Wise Aanswer Draft Of Issues",
  //     flex: 1,
  //     align: "center",
  //     headerAlign: "center",
  //   },
  ...
  //             size="small"
  //             onChange={(e) => {
  //               console.log("loge", e.target.value, record.row);
  //               setCheck(e.target.value);
  //             }}
  //             placeholder="Enter Response To Notice"
  //           />
  //           <Button
  //             variant="outlined"
  //             onClick={() => {
  //               console.log("cjjj", check);
  //             }}
  //           >
  //             Save
  //           </Button>
  ...
  useEffect(() => {
    ...
    //   department: router.query.departmentName,
  ...
  const getNoticeDetails = () => {
    axios.get(`${urls.LCMSURL}/notice/getTrnNoticeData`).then((res) => {
      console.log(res);
      setDataSource(
        res.data.map((r, i) => ({
          srNo: i + 1,
  ...
  const onSubmitForm = (fromData) => {
    const finalBody = {
      ...fromData,
      // responseToNoticeDaoLst : paragraphWiseAanswerDraftOfIssues,
      id: Number(selectedNotice.id),
      // id: Number(noticeId),
      pageMode:
        window.event.submitter.name == "save"
          ? "RESPONSE_TO_NOTICE_CREATE"
          : "RESPONSE_TO_NOTICE_DRAFT",
          timeStamp: moment(new Date()).unix().toString(),
    };

    //responseToNoticeDaoLst
    console.log(
      "Form Data",
      fromData,
      "data",
      window.event.submitter.name,
      "finalBody",
      finalBody
    );
    ...
    // axios
    //   .post(
    //     `http://localhost:8098/lc/api/parawiseReport/saveParawiseReport`,
    //     finalBody
    //   )

    axios
      .post(`${urls.LCMSURL}/notice/saveTrnNotice`, finalBody, {
        headers: {
            ...
            <div>
              <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit(onSubmitForm)}>
                  ...
                  {/* Parawise Report */}
                  <Paper
                    className={styles.parawisePaper}
                    style={{
                            ...
                            >
                              ...
                              >
                                <TextField
                                  ...
                                  type="number"
                                  {...register(
                                    `responseToNoticeDaoLst.${index}.issueNo`
                                  )}
                                />
                              </Grid>
                              <Grid
                              ...
                              >
                                <TextField
                                  ...
                                  size="small"
                                  {...register(
                                    `responseToNoticeDaoLst.${index}.paragraphWiseAanswerDraftOfIssuesOg`
                                  )}
                                />
                              </Grid>
                              <Grid
                              ...
                              >
                                <TextField
                                  ...
                                  size="small"
                                  {...register(
                                    `responseToNoticeDaoLst.${index}.paragraphWiseAanswerDraftOfIssues`
                                  )}
                                />
                              </Grid>
                              <Grid
