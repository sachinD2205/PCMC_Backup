...
const View = () => {
  ...
  const { reset, setValue, getValues, method, watch } = methods;
  ...
  console.log("handleNext",activeStep);
    const finalBody={
      ...data,
      mstAdvocateAttachmentDao:JSON.parse(localStorage.getItem("mstAdvocateAttachmentDao"))
    }
    console.log("finalBodyAdvocate",finalBody)
    if (activeStep == steps.length - 1) {
    axios.post(`${urls.LCMSURL}/master/advocate/save`,finalBody
    ).then((res) => {
              ...
              <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit(handleNext)}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
                    <Button disabled={activeStep.length === 0} variant='outlined' onClick={() => previousStep()}>
