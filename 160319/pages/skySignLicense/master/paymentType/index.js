...
// import schema from "./paymentTypeschema";
...
const Index = () => {
  ...
  const getDataById = (value) => {
    ...
    axios
      .get(
        `${urls.SSLM}master/MstPaymentType/getpaymentTypeDaoDataById/?id=${value}`
      )
      .then((res) => {
        reset(res.data);
  ...
  const deleteById = (value) => {
    swal({
      ...
      if (willDelete) {
        axios
          .delete(
            `${urls.SSLM}master/MstPaymentType/discardpaymentType/${value}`
          )
          .then((res) => {
            if (res.status == 226) {
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      ...
      axios
        .post(
          `${urls.SSLM}master/MstPaymentType/savepaymentType`,
          finalBodyForApi
        )
        .then((res) => {
          if (res.status == 201) {
    ...
    // Update Data Based On ID
      ...
      axios
        .put(
          `${urls.SSLM}master/MstPaymentType/editpaymentType/?id=${id}`,
          finalBodyForApi
        )
        .then((res) => {
          if (res.status == 200) {
