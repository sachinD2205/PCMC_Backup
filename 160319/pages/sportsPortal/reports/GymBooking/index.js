...
const Index = () => {
  ...
  const searchButton = () => {
    ...
    axios
      .get(
        ...
        `${URLS.SPURL}/gymBooking/getDataByBookingDate?fromDate=${selectedFromDate}&toDate=${selectedToDate}`
      )
      .then((res) => {
        console.log("response123", res);
        res.data.gymBooking.length === 0 &&
          toast("No Gym Bookings Available !", {
            type: "warn",
          });

        setSearchResults(res.data.gymBooking);
      })
      .catch((err) => {
  ...
  const columns = [
    ...
    // {
    //   field: 'bankAccountHolderName',
    //   headerName: 'Bank Account HolderName',
    //   //type: "number",
    //   flex: 1,
    // },
