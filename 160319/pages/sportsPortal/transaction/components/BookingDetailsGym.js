...
const BookingDetailsGym = () => {
  const {
    control,
    register,
    reset,
    formState: { errors },
  } = useF({ resolver: yupResolver(schema) }); // useFormContext();
  ...
  const getAllGymData = () => {
    axios
      ...
      .catch((err) =>
        console.log("while getting all gym data error is :" + err)
      );
  };
            ...
            <Controller
              render={({ field }) => (
                ...
                >
                  ...
                  {/* // from Api data this menus/ options to be displayed. */}
                </Select>
              )}
              name="bookingType"
              control={control}
