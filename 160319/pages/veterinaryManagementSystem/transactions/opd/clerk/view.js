...
import { Paper, Button, MenuItem, Select, InputLabel, TextareaAutosize } from "@mui/material";
...
const Index = () => {
  ...
  let opdSchema = yup.object().shape({
    opdKey: yup.number().required("Please select an OPD"),
    licenseNo: yup.string().required("Please enter building name").default("A123"),
    ownerFullName: yup.string().required("Please enter owner's full name").default("Karan"),
    ownerAddress: yup.string().required("Please enter owner's address"),
    emailAddress: yup.string().required("Please enter an email id"),
    mobileNumber: yup.string().required("Please enter a mobile no."),
    ...
    animalName: yup.string().required("Please select an animal"),
    animalAge: yup.number().required("Please enter age of the animal"),
    animalSex: yup.string().required("Please select a gender"),
    animalSpeciesKey: yup.number().required("Please select a gender"),
    animalColour: yup.string().required("Please enter the color"),
    ...
    // treatmentCategory: yup.string().required("Please select a treatment category"),
    // aknowledgementSlipNo: yup
    //   .string()
    //   .required("Please enter acknowledgement slip no.")
    ...
    // receiptNo: yup.string().required("Please enter receipt no."),
    // receiptDate: yup.string().required("Please select a receipt date"),
    // narration: yup.string().required("Please enter a narration"),
    // accountGlCode: yup.string().required("Please enter account Gl code"),
  });
  ...
  useEffect(() => {
    setValue("transactionDate", moment(new Date(), "YYYY-MM-DD").format("YYYY-MM-DD"));
  ...
  const payment = () => {
    ...
    axios
      .post(`${URLs.VMS}/trnAnimalTreatment/save`, bodyForAPI)
      .then((res) => {
      ...
      <Paper className={styles.main}>
        ...
        <form onSubmit={handleSubmit(finalSubmit)} style={{ padding: "5vh 3%" }}>
          <div className={styles.row}>
            <FormControl disabled={router.query.id} variant="standard" error={!!error.opdKey}>
