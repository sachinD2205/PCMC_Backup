...
const Index = () => {
        ...
        return (
          <>
            <IconButton
              ...
              onClick={() => {
                router.push({
                  pathname: `/veterinaryManagementSystem/transactions/opd/doctor/view`,
                  query: {
                    id: params.row.id,
  ...
  const deleteApplication = (deleteId) => {
    sweetAlert({
      ...
      if (willDelete) {
        axios.delete(`${URLs.VMS}/trnAnimalTreatment/delete/${deleteId}`).then((res) => {
          if (res.status == 226) {
  ...
  return (
    <>
      ...
      <Paper className={styles.main}>
        <div className={styles.title}>Treating sick and injured animal through OPD</div>
        {/* <div className={styles.row} style={{ justifyContent: "flex-end" }}>
          <Button
            variant="contained"
            endIcon={<Add />}
            onClick={() => {
              router.push(`${URLs.APPURL}/veterinaryManagementSystem/transactions/opd/clerk/view`);
            }}
          >
