...
import subjectSchema from "../../../../containers/schema/school/transactions/subjectSchema";
...
const Index = () => {
  ...
  const getSubjectMaster = (_pageSize = 10, _pageNo = 0, _sortBy = "id", _sortDir = "desc") => {
    // console.log("_pageSize,_pageNo", _pageSize, _pageNo);
    axios
      .get(`${urls.SCHOOL}/mstSubject/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
          sortBy: _sortBy,
      ...
      .then((r) => {
        ...
        let result = r.data.mstSubjectList;
        console.log("mstSubjectList", result);
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      const tempData = axios.post(`${urls.SCHOOL}/mstSubject/save`, _body).then((res) => {
        if (res.status == 201) {
    ...
    // Update Data Based On ID
      ...
      axios.post(`${urls.SCHOOL}/mstSubject/save`, _body).then((res) => {
        console.log("res", res);
        if (res.status == 201) {
          fromData.id
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.SCHOOL}/mstSubject/save`, body).then((res) => {
    ...
    >
      ...
      {showTable && (
        <DataGrid
          ...
          columns={columns}
          onPageChange={(_data) => {
            getSubjectMaster(data.pageSize, _data);
          }}
          onPageSizeChange={(_data) => {
            console.log("222", _data);
            // updateData("page", 1);
            getSubjectMaster(_data, data.page);
          }}
        />
      )}
    </Paper>
