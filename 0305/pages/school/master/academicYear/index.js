...
import { Box, Button, FormControl, FormHelperText, Grid, Paper, Slide, TextField } from "@mui/material";
...
const Index = () => {
      ...
      .then((r) => {
        ...
        let _res = result.map((r, i) => {
          ...
          return {
            ...
            academicYearFrom: moment(r.academicYearFrom, "YYYY-MM-DD").format("DD-MM-YYYY"),

            academicYearTo: moment(r.academicYearTo, "YYYY-MM-DD").format("DD-MM-YYYY"),
  ...
  const onSubmitForm = (fromData) => {
    ...
    let _body = {
      ...fromData,
      academicYearFrom: moment(fromData.academicYearFrom, "YYYY-MM-DD").format("YYYY-MM-DD"),
      academicYearTo: moment(fromData.academicYearTo, "YYYY-MM-DD").format("YYYY-MM-DD"),
      activeFlag: fromData.activeFlag,
    };
    if (btnSaveText === "Save") {
      const tempData = axios.post(`${urls.SCHOOL}/mstAcademicYear/save`, _body).then((res) => {
        if (res.status == 201) {
    ...
    // Update Data Based On ID
      ...
      axios.post(`${urls.SCHOOL}/mstAcademicYear/save`, _body).then((res) => {
        console.log("res", res);
        if (res.status == 201) {
          fromData.id
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.SCHOOL}/mstAcademicYear/save`, body).then((res) => {
