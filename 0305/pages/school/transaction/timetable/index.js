...
const Index = () => {
  ...
  const updateTimetableList = (rowData, action) => {
    ...
    let tableRow = {
      ...
      periodDao: {
        monDay: subjectList.find((subject) => subject.value === rowData?.monDay)?.label,
        tuesDay: subjectList.find((subject) => subject.value === rowData?.tuesDay)?.label,
        wednesDay: subjectList.find((subject) => subject.value === rowData?.wednesDay)?.label,
        thursday: subjectList.find((subject) => subject.value === rowData?.thursday)?.label,
        friDay: subjectList.find((subject) => subject.value === rowData?.friDay)?.label,
        saturDay: subjectList.find((subject) => subject.value === rowData?.saturDay)?.label,
      },
    };
    ...
    if (action == "delete") {
      ...
      // @ts-ignore
      if (timeTable) {
        console.log("TimeTable last id: ", timeTable[timeTable?.length - 1]?.id + 1);
        setNewStateTableData((oldData) => {
          return [...oldData, tableRow];
        });
        setTimeTable((oldData) => {
          return [
            ...oldData,
            {
              fromTime: moment(rowData.fromTime).format("HH:mm a"),
              ...
              saturDay: subjectList.find((subject) => subject.value === rowData?.saturDay)?.label,
              // @ts-ignore
              id: (timeTable[timeTable?.length - 1]?.id ?? 0) + 1,
            },
          ];
        });
      }
  ...
  const finalSubmit = (data) => {
    ...
    let _newStateTableData = newStateTableData?.map((data) => {
      return {
        id: data?.id,
        fromTime: data?.fromTime,
        toTime: data?.toTime,
        activeFlag: data?.activeFlag,
        periodDao: data?.periodDao,
      };
    });
    let _teacherSub = timeTableTeachersList?.map((data) => {
      return {
