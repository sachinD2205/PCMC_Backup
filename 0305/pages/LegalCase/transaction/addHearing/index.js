      ...
      .then((r) => {
        ...
        let _res = result.map((r, i) => {
          ...
          return {
            ...
            caseMainType: caseTypes?.find((obj) => obj.id === r.caseMainType)?.caseMainType,
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.LCMSURL}/trnsaction/addHearing/save`, body).then((res) => {
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => {
        return (
          <Box>
            {console.log("22", language === "en" ? "caseMainType" : "caseMainTypeMr")}
            <IconButton
