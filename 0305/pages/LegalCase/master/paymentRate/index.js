...
const Index = () => {
      ...
      .then((r) => {
        ...
        let _res = result.map((r, i) => {
          ...
          return {
            ...
            caseMainType: caseMainTypes?.find((obj) => obj?.id === r.caseMainTypeId)?.caseMainType,

            caseMainTypeMr: caseMainTypes?.find((obj) => obj?.id === r.caseMainTypeId)?.caseMainTypeMr,

            caseSubType: r.caseSubType,

            subType: caseSubTypes?.find((obj) => obj?.id === r.caseSubType)?.subType,

            caseSubTypeMr: caseSubTypes?.find((obj) => obj?.id === r.caseSubType)?.caseSubTypeMr,
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.LCMSURL}/master/paymentRate/save`, body).then((res) => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      const tempData = axios.post(`${urls.LCMSURL}/master/paymentRate/save`, _body).then((res) => {
        if (res.status == 200) {
    ...
    // Update Data Based On ID
      ...
      const tempData = axios.post(`${urls.LCMSURL}/master/paymentRate/save`, _body1).then((res) => {
        console.log("res", res);
        if (res.status == 200) {
          fromData.id
                ...
                <Slide direction="down" in={slideChecked} mountOnEnter unmountOnExit>
                  <Grid container sx={{ marginLeft: "30px" }}>
                    <Grid item xs={12} sm={6} md={4} lg={4} xl={2}>
                      <FormControl sx={{ marginTop: 2 }} error={!!errors.caseMainTypeId}>
                        ...
                        <Controller
                          render={({ field }) => (
                            <Select
                              ...
                              InputLabelProps={{
                                //true
                                shrink: (watch("T") ? true : false) || (router.query.T ? true : false),
                              }}
                            >
                              {caseMainTypes &&
                        ...
                        />
                        <FormHelperText>
                          {errors?.caseMainTypeId ? errors.caseMainTypeId.message : null}
                        </FormHelperText>
                    ...
                    <Grid item xs={12} sm={6} md={4} lg={4} xl={2}>
                      <FormControl sx={{ marginTop: 2 }} error={!!errors.caseSubType}>
                        <InputLabel id="demo-simple-select-standard-label">
                              ...
                              {caseSubTypes &&
                                caseSubTypes.map((subType, index) => (
                                  ...
                                  >
                                    {/* @ts-ignore */}
                                    {/* {title.title} */}
                                    {language == "en" ? subType?.subType : subType?.caseSubTypeMr}
                                  </MenuItem>
                                ))}
                            </Select>
                          )}
                    ...
                    <Grid item xs={12} sm={6} md={4} lg={4} xl={2}>
                      <TextField
                        ...
                        InputLabelProps={{
                          //true
                          shrink: (watch("rate") ? true : false) || (router.query.rate ? true : false),
                        }}
                        {...register("rate")}
                        error={!!errors.rate}
