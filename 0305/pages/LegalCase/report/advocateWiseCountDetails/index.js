...
const Index = () => {
  ...
  const searchButton = async () => {
    ...
    try {
      axios
        .get(
          `${urls.LCMSURL}/report/getAdvocatewiseCountReportV2?fromDate=${fromDate}&toDate=${toDate}&advocateId=${selectedID1}`,
        )
        .then((r) => {
          console.log("Res", r);
          setDataSource(
  ...
  return (
    ...
    >
      <Grid container gap={4} direction="column">
        ...
        >
          <Grid item>
            <h2 style={{ marginBottom: 0 }}>{labels.advWiseCountDetails}</h2>
          </Grid>
        </Grid>
        <form>
                      ...
                      {this.props.dataToMap &&
                        this.props.dataToMap.map((r, i) => (
                          <tr key={i}>
                            <td>{r.srNo}</td>
                            {/* <td>{r.advName}</td> */}
                            <td>{language == "mr" ? r.advNameMr : r.advName}</td>
