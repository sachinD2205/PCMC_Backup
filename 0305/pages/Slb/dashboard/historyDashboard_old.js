...
const Index = () => {
    ...
    axios
      ...
      .then((res) => {
        const data = res?.data.parameterList.map((r, i) => ({
          ...
          lastActualBenchmarkValue: parseFloat(
            r?.lastActualBenchmarkValue
          ).toFixed(0),
  ...
  const getBenchmarkHistoryByParameter = (pmkey) => {
    ...
    axios
      .get(
        `${urls.SLB}/benchmarkHistory/getAllByParameterId?parameterId=${paramaterKey}`
      )
      .then((res) => {
        const data = res?.data.benchmarkHistoryList.map((r, i) => ({
          ...
          parameterName: r?.parameterName,
          benchmarkValue: r?.benchmarkValue,
          calculatedBenchmarkValue: parseFloat(
            r?.calculatedBenchmarkValue
          ).toFixed(0),
          benchmarkDate: r?.benchmarkDate,

          // convert bemchmarkDate to dd/mm/yyyy HH:mm:ss format
          benchmarkDateFormatted: new Date(r?.benchmarkDate).toLocaleDateString(
            "en-GB",
            {
              day: "numeric",
              month: "short",
        ...
        const parameterKeys = [
          ...new Set(data.map((item) => item.parameterKey)),
        ];
              ...
              series={[
                {
                  ...
                  data: benchmarkHistory1.map((r) => {
                    return {
                      x: r.benchmarkDateFormatted
                        ? r.benchmarkDateFormatted
                        : r.id,
                      y: r.benchmarkValue,
                    };
                  }),
