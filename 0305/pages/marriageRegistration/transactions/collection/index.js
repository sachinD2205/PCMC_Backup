...
const Index = () => {
  ...
  const getcollectionDetails = () => {
    axios
      // .get(`http://localhost:8090/cfc/api/collection/getcollectionDetails`)
      .then((r) => {
        setDataSource(
          r.data.map((j, i) => ({
            id: j.id,
  ...
  const editForm = async () => {
    ...
    if (okText === 'Save') {
      await axios
        // .put(
        //   `http://localhost:8090/cfc/api/collection/updateCollection`,
        //   parametersForEdit,
        // )
        .then((res) => {
          if (res.status === 200) {
  ...
  const deleteForm = async (record) => {
    ...
    await axios
      // .delete(
      //   `http://localhost:8090/cfc/api/collection/deleteCollection/${recordId}`,
      // )
      .then((res) => {
        if (res.status == 226) {
          ...
          <Row style={{ marginBottom: '10px' }}>
            ...
            <Col xl={2} lg={2} md={2} sm={4} xs={4}>
              <Button
                ...
                onClick={() =>
                  router.push(
                    `/marriageRegistration/transactions/collection/collection`,
                  )
                }
              >
                Add
