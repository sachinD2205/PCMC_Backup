...
const LoiGenerationComponent = (props) => {
  ...
  const getServiceCharges = () => {
    axios
      ...
      //   )}`,
      // )
      .get(`${urls.LMSURL}/libraryRateCharge/getByServiceId?serviceId=${getValues('serviceId')}`)
      .then((r) => {
        console.log("Service charges", r.data.mstLibraryRateChargeList);
        let tempList = r.data.mstLibraryRateChargeList;
        let totalServiceChargeL = []
        console.log("libraryType123", libraryType);
        tempList.forEach((service) => {
          if (libraryType == 'L') {
            if (service.libraryType == 'L') {
              totalServiceChargeL.push(service)
            }
          }
          else if (libraryType == 'C') {
            if (service.libraryType == 'C') {
              totalServiceChargeL.push(service)
            }
          }
        })
        setValue('serviceCharges', totalServiceChargeL)
        setServiceCharges(totalServiceChargeL)
      ...
      axios
        ...
        .then((r) => {
          ...
          if (r.status === 200) {
            ...
            setLibraryType(libraryList.find((library) => library.id == r.data.libraryKey)?.libraryType)
            console.log("hotay change", libraryList, libraryList.find((library) => library.id == r.data.libraryKey)?.libraryType);
          }
        })
    }
  }
  ...
  useEffect(() => {
    getServiceCharges()
  }, [serviceId, libraryType, tempMonths, watch('serviceName')])
  ...
  useEffect(() => {
    ...
    if (tempMonths == 12) {
      if (libraryType == 'L') {
        setValue('loi.amount', Number(200 + deposit))
        setValue('loi.totalInWords', toWords.convert(Number(200 + deposit)))
      }
      else {
        setValue('loi.amount', Number((total * tempMonths) + deposit))
        setValue('loi.totalInWords', toWords.convert(Number((total * tempMonths) + deposit)))
      }
    }
    else {
      setValue('loi.amount', Number((total * tempMonths) + deposit))
  ...
  const handleNext = (data) => {
    ...
    axios
      .post(
        `${urls.LMSURL}/trnApplyForNewMembership/generateLoi`,
        finalBodyForApi,
      )
      .then((res) => {
        ...
        if (res.status == 200 || res.status == 201) {
          ...
          router.push({
            pathname:
              '/lms/transactions/newMembershipRegistration/scrutiny/LoiGenerationReciptmarathi',
            query: {
              id: router?.query?.id,
            },
          })
        ...
        <form onSubmit={handleSubmit(handleNext)}>
          ...
          <div>
            <Typography sx={{ fontWeight: 800, color: 'red', marginLeft: "7vh", marginTop: "2vh", marginBottom: "2vh" }}>
              ...
              *Service Charge Type Name conventions 1) C :- Charge 2) D :- Deposit
            </Typography>
          </div>
                  ...
                  >
                    <Grid item xs={4} md={4} sm={4} xl={4} lg={4}>
                      <TextField
                        ...
                        {...register(
                          `serviceCharges.${index}.chargeType`,
                        )}
