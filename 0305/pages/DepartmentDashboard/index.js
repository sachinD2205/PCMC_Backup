...
import { logout, setSelectedApplicationId } from "../../features/userSlice";
...
const DepartmentDashboard = (props) => {
  ...
  const usersDepartmentDashboardData = useSelector((state) => {
    console.log("dataaaaa", state.user.usersDepartmentDashboardData);
  ...
  const abcd = arr.map((val) => {
    let childEle = usersDepartmentDashboardData.menus.filter((value) => val.id == value.parentId);
    // res.push(val);
    res.push(...childEle);
    return val;
  });
        ...
        <Container maxWidth="xl" minHeight="sm">
          <Toolbar disableGutters>
            ...
            <Box sx={{ display: { xs: "none", md: "flex" } }}>
              <Image src="/logo.png" alt="Picturer" width={52} height={52} style={{ cursor: "pointer" }} />
            </Box>
            ...
            {/* User Login Logout */}
            <Box sx={{ flexGrow: 6, display: { xs: "none", md: "flex" } }}></Box>
            <Box sx={{ flexGrow: 1, display: { xs: "none", md: "flex" } }}>
              <Button onClick={handleCloseNavMenu}>
                <Button onClick={chnageLaguage}>{lang ? <English /> : <Marathi />}</Button>
              </Button>
            </Box>
            ...
            <Box sx={{ flexGrow: 0 }}>
              ...
              <Tooltip title="Open settings">
                <IconButton onClick={handleOpenUserMenu} sx={{ paddingLeft: 1 }}>
                  <AccountCircleIcon sx={{ fontSize: "31px", color: "white" }} />
                </IconButton>
              </Tooltip>
              ...
              >
                ...
                <MenuItem className={styles.menuList} onClick={handleCloseUserMenu}>
                  <Box
                    sx={{
                      width: "100%",
                    }}
                  ...
                  <Box className={styles.menuTitleContainer}>
                    <Typography sx={{ color: "blue", fontSize: "12px" }}>{user?.userDao?.email}</Typography>
                  </Box>
                </MenuItem>
                <MenuItem>
                  <Button
        ...
        >
          {usersDepartmentDashboardData.applications.length > 0
            ? usersDepartmentDashboardData.applications.map((data, index) => {
                ...
                console.log("colorForMenu", colorForMenu);
                return (
                  <Grid
                    item
                    xs={3}
