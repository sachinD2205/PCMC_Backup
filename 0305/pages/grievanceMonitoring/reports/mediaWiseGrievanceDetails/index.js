...
const Index = () => {
  ...
  const onSubmitFunc = () => {
    if (watch("mediaId") && watch("fromDate") && watch("toDate")) {
      ...
      ///////////////////////////////////////////
      axios
        .post(`${urls.GM}/report/getMediaWiseGrievanceReport`, apiBodyToSend)
        .then((res) => {
          console.log(":log", res);
          setLoading(true);
          if (res?.status === 200 || res?.status === 201) {
  ...
  const columns = [
    ...
    {
      field: language === "en" ? "complaintSubject" : "complaintSubjectMr",
  ...
  function generateCSVFile(data) {
    ...
    downloadLink.download = data ? `${data[0]?.mediaName}.csv` : "data.csv";
    downloadLink.click();
        ...
        >
          ...
          >
            <strong className={styles.fancy_link1}>
              <FormattedLabel id="MediaWiseGrievanceDetails" />
            </strong>
          </Box>
        </Box>
        {/* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> */}
          ...
          <Paper elevation={3} style={{ margin: "10px", width: "80%" }}>
            <form onSubmit={handleSubmit(onSubmitFunc)}>
              ...
              >
                ...
                >
                  <FormControl error={!!errors.mediaId}>
                    <InputLabel>
                      <FormattedLabel id="mediaName" />
                    </InputLabel>
                    <Controller
                      render={({ field }) => (
                        <Select
