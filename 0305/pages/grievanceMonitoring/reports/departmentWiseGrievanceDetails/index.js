...
const Index = () => {
  ...
  const onSubmitFunc = () => {
    if (watch("department") && watch("fromDate") && watch("toDate")) {
      ...
      ///////////////////////////////////////////
      axios
        .post(`${urls.GM}/report/getDeaprtmentWiseGrievanceCount`, apiBodyToSend)
        .then((res) => {
          console.log(":log", res);
          setLoading(true);
          if (res?.status === 200 || res?.status === 201) {
  ...
  function generateCSVFile(data) {
    ...
    downloadLink.download = data ? `${data[0]?.departmentName}.csv` : "data.csv";
    downloadLink.click();
        ...
        >
          ...
          >
            <strong className={styles.fancy_link1}>
              <FormattedLabel id="DepartmentWiseGrievanceDetails" />
            </strong>
          </Box>
        </Box>
        {/* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> */}
