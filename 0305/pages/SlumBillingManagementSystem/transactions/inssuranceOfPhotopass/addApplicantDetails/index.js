...
const Index = () => {
  ...
  const handlePreviewButton = (type) => {
    if (type === "selfDeclarationForm") {
      router.push(
        "/SlumBillingManagementSystem/transactions/inssuranceOfPhotopass/generateDocuments/selfDeclaration",
      );
      // var table = document.getElementById("selfDeclaration").outerHTML;
      //   var newWindow = window.open();
      //   newWindow.document.write(table);
    } else {
      router.push(
        "/SlumBillingManagementSystem/transactions/inssuranceOfPhotopass/generateDocuments/selfAttestation",
      );
      // var table = document.getElementById("selfAttestation").outerHTML;
      //   var newWindow = window.open();
      //   newWindow.document.write(table);
    }
  };
  ...
  const getSlumData = () => {
    axios.get(`${urls.SLUMURL}/mstSlum/getAll`).then((r) => {
      ...
      setValue("slumKey", !temp ? "-" : language == "en" ? temp?.slumEn : temp?.slumEn);
      setSlumDropDown(res);
    });
  };
        ...
        const tempData = axios
          ...
          .then((res) => {
            ...
            if (res.status == 201) {
                    sweetAlert({
                        title: "Saved!",
                        text: "Photopass Application Saved successfully !",
                        icon: "success",
                        dangerMode: false,
                        ...
                        if (will) {
                            sweetAlert({
                                // title: "Great!",
                                text: ` Your Issuance of Photopass Application No Is : ${res.data.message.split('[')[1].split(']')[0]}`,
                                icon: "success",
      ...
      >
        <form onSubmit={handleSubmit(handleOnSubmit)}>
          ...
          {/* <Paper style={{ display: "none" }}>
            {console.log("selectedHutData", selectedHutData)}
            {choice && choice === "selfDeclaration" && selectedHutData && (
              <SelfDeclaration applicantData={applicantData} hutData={selectedHutData}  />
            )}
            {choice && choice === "selfAttestation" && selectedHutData && (
              <SelfAttestation applicantData={applicantData} hutData={selectedHutData} />
            )}
          </Paper> */}
        </form>
      </Paper>
