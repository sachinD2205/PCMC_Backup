...
const Index = () => {
  ...
  const getAllForBilling = (year) => {
    //Table
    axios.get(`${urls.SLUMURL}/trnBill/getBillsByYear?year=${year}`, config).then((res) => {
      setTable(
  ...
  const columns = [
    ...
    },
    {
      ...
      field: "bill",
      headerAlign: "center",
      headerName: <FormattedLabel id="bill" />,
      flex: 1,
      renderCell: (params) => {
        return (
          <Button
            ...
            onClick={() => {
              router.push({
                pathname: "/SlumBillingManagementSystem/transactions/hutBillGeneration/bill",
