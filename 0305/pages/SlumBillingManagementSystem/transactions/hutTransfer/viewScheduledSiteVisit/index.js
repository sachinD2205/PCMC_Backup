...
const index = () => {
  ...
  useEffect(() => {
    if(router.query.id){
      getSiteVisitDataById(router.query.id);
    }
  }, [router.query.id]);

  useEffect(() => {
    let res = dataSource && dataSource?.find((obj)=> obj.referenceKey == router.query.id );
    // console.log("res",res)
      setValue("scheduledTime", res? res?.scheduledDate : "-")
      setValue("visitTime", res? res?.visitDate : "-")
  }, [dataSource]);

  const getSiteVisitDataById = (id) => {
    if (loggedInUser === "citizenUser") {
        axios
        .get(`${urls.SLUMURL}/trnVisitSchedule/getAll`, {
          headers: {
            UserId: user.id
          },
        })
        .then((r) => {
          let result = r.data.trnVisitScheduleList;
            console.log("getSiteVisitDataById", result);
  ...
  const handleOnSubmit = (formData) => {
    const tempData = axios
      ...
      .then((res) => {
        if (res.status == 201) {
          sweetAlert("Saved!", `Site Visit ${dataSource.applicationNo} scheduled successfully !`, "success");
  ...
  return (  
    ...
    >
      <form onSubmit={handleSubmit(handleOnSubmit)}>
        ...
        >
          <h2>
            <FormattedLabel id="scheduleSiteVisitDetails" />
          </h2>
        </Box>
