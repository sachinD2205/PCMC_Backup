...
const Index = () => {
  ...
  const schema = yup.object().shape({
    roadTypeName: yup.string().required("Road Type Name is required"),
    roadTypePrefix: yup.string().required("Road Type Prefix is required"),
  });
  ...
  // Get Table - Data
  const getRoadTypes = (_pageSize = 10, _pageNo = 0) => {
    console.log("_pageSize,_pageNo", _pageSize, _pageNo);
    axios
      .get(`${urls.REURL}/mstRoadType/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      const tempData = axios.post(`${urls.REURL}/mstRoadType/save`, _body).then((res) => {
        if (res.status == 201) {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.REURL}/mstRoadType/save`, body).then((res) => {
  ...
  const columns = [
    ...
    // { field: "courtNo", headerName: "Court No", flex: 1 },
    {
      ...
      headerName: <FormattedLabel id="roadType" />,
      flex: 1,
    },
    {
      // field: "billingCycle",
      field: "roadTypePrefix",
        ...
        <form onSubmit={handleSubmit(onSubmitForm)}>
          {isOpenCollapse && (
            <Slide direction="down" in={slideChecked} mountOnEnter unmountOnExit>
              <Grid container>
                <Grid item xs={2} sm={1.5} md={1} lg={1} xl={1}></Grid>
                <Grid item xs={10} sm={5} md={3} lg={3} xl={3}>
                  <TextField
                    label={<FormattedLabel id="roadTypeName" />}
                    id="standard-basic"
                    variant="standard"
                    {...register("roadTypeName")}
                    error={!!errors.roadTypeName}
                    InputProps={{ style: { fontSize: 18 } }}
                    InputLabelProps={{
                      style: { fontSize: 15 },
                      //true
                      // shrink:
                      //   (watch("roadTypeName") ? true : false) || (router.query.roadTypeName ? true : false),
                    }}
                    helperText={errors?.roadTypeName ? errors.roadTypeName.message : null}
                  />
                </Grid>
