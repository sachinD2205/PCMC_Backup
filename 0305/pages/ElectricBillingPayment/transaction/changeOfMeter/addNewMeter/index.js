...
const Index = () => {
  ...
  useEffect(()=>{
    ...
    getMeterStatusData(_res?.meterStatusKey)
    
  },[oldMeterReadingData])

  const getMeterStatusData = (meterStatusKey) => {
    axios.get(`${urls.EBPSURL}/mstMeterStatus/getAll`).then((res) => {
      let temp = res.data.mstMeterStatusList;
      let result = temp && temp.find((each) => each.id === meterStatusKey)
      // console.log("getMeterStatusData", result);
    setValue("oldMeterStatusKey", result?.meterStatus ? result?.meterStatus : "-");
  ...
  const getMeterStatus = () => {
    axios.get(`${urls.EBPSURL}/mstMeterStatus/getAll`).then((res) => {
      let temp = res.data.mstMeterStatusList;
      // console.log("getMeterStatus", temp);
      setMeterStatusDropDown(temp);
    });
  }
        ...
        .then((r) => {
          ...
          let _res = result.map((r, i) => {
            ...
            return {
              ...
              oldBillData: r.trnMeterReadingAndBillGenerateList,
              meterNo: r.meterNo,
            };
          });
  ...
  const handleSelectConnection = (id, oldBillData) => {
    // console.log("selected oldBillData", oldBillData[oldBillData.length-1]);
    let letestBillData = oldBillData && oldBillData[oldBillData.length-1]
    setOldMeterReadingData(letestBillData);
          ...
          <Grid container sx={{ padding: "10px" }}>
            ...
            >
              <TextField
                ...
                sx={{ m: 1, minWidth: "50%" }}
                variant="outlined"
                value={watch("meterConnectionDate")}
                InputLabelProps={{
                  //true
                  shrink:
