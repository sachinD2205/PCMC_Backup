...
const BachatGatCategory = () => {
  ...
  const getBachatgatCategoryTrn = (_pageSize = 10, _pageNo = 0) => {
    {
      loggedUser === "citizenUser"
        ? axios
          .get(`${urls.BSUPURL}/trnBachatgatRegistration/getAllModificationList`, {
            headers: {
              UserId: user.id
            },
            params: {
        ...
        : axios
          .get(
            `${urls.BSUPURL}/trnBachatgatRegistration/getAllModificationList`,
            {
              headers: {
  ...
  const columns = [
    ...
    {
      field: "createdDate",
      headerName: <FormattedLabel id="modificationDate" />,
      width: 200,
      align: "left",
      headerAlign: "center",
    },
            ...
            <IconButton
              onClick={() => {
                ((loggedUser == "citizenUser" || loggedUser === "cfcUser") && params.row.statusVal == 1) ? router.push({
                  pathname: "/BsupNagarvasthi/transaction/bachatgatModification/edit",
                  query: {
                    id: params.row.id,
                  },
                }) :
                  router.push({
                    pathname: "/BsupNagarvasthi/transaction/bachatgatModification/view",
                    query: {
                      id: params.row.id,
                    },
                  });
            ...
            {((loggedUser == "citizenUser" || (loggedUser === "cfcUser")) && params.row.statusVal === 2) && (<IconButton
              onClick={() => {
                router.push({
                  ...
                  query: {  id: params.row.applicationNo, trn: "M" },
                })
              }}
            >
