...
const getAllSubSchemes = (_pageSize = 10, _pageNo = 0) => { axios.get(`${urls.BSUPURL}/mstSubSchemes/getAll`, {
  ...
  const onSubmitFunc = (formData) => {
    if (watch("fromDate") && watch("toDate")) {
      ...
      ///////////////////////////////////////////
      axios
        .post(`${urls.BSUPURL}/report/getNamunaCReport`, apiBodyToSend)
        .then((res) => {
          console.log(":log", res);
          setLoading(true);
            ...
            if (res?.data.length > 0) {
              setData(
                res?.data?.map((r, i) => ({
                  ...
                  reasonForNonEligible: r.reasonForNonEligible,
  ...
  const columns = [
    ...
    },
    {
      ...
      {
        field: "reasonForNonEligible",
        headerName: <FormattedLabel id="reasonForNonEligible" />,
        minWidth: 400,
        headerAlign: "center",
        align: "center",
      },
  ...
  function generateCSVFile(data) {
    ...
    downloadLink.download = data ? `Namuna C Report.csv` : "data.csv";
    downloadLink.click();
        ...
        >
          ...
          >
            <strong className={styles.fancy_link1}>
              <FormattedLabel id="namunaCReportHeading" />
            </strong>
          </Box>
        </Box>
        {/* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> */}
