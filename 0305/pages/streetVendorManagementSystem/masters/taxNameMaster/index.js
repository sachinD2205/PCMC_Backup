...
import styles from "../../../../components/streetVendorManagementSystem/styles/taxName.module.css";
import schema from "../../../../components/streetVendorManagementSystem/schema/TaxNameSchema";
const Index = () => {
  const {
    register,
    control,
  ...
  const getTaxTypes = () => {
    axios.get(`${urls.HMSURL}/taxTypeMaster/getTaxTypeMasterData`).then((r) => {
      setTaxTypes(
        r.data.map((row) => ({
          id: row.id,
          taxType: row.taxType,
  ...
  const getTaxName = () => {
    axios
      .get(`${urls.HMSURL}/taxNameMaster/getTaxNameMasterData`)
      .then((res) => {
        setDataSource(
          res.data.map((r, i) => ({
            id: r.id,
            ...
            taxTypeName: taxTypes?.find((obj) => obj?.id === r.taxType)
              ?.taxType,
  ...
  const onSubmitForm = (formData) => {
    ...
    axios
      .post(`${urls.HMSURL}/taxNameMaster/saveTaxNameMaster`, finalBodyForApi)
      .then((res) => {
        if (res.status == 201) {
          formData.id
  ...
  const deleteById = (value) => {
    swal({
      ...
      axios
        .delete(`${urls.HMSURL}/taxNameMaster/discardTaxNameMaster/${value}`)
        .then((res) => {
          if (res.status == 226) {
            if (willDelete) {
                              ...
                              >
                                {taxTypes &&
                                  taxTypes.map((taxType, index) => (
                                    <MenuItem key={index} value={taxType.id}>
                                      {taxType.taxType}
                                    </MenuItem>
                                  ))}
                              </Select>
