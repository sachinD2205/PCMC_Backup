...
import schema from "../../../../components/streetVendorManagementSystem/schema/HawkerModeSchema";
import { useSelector } from "react-redux";

// Fun
// http://localhost:4000/streetVendorManagementSystem/masters/hawkerMode
const Index = () => {
  const {
    register,
    control,
  ...
  const columns = [
    ...
    {
      field: "hawkerModePrefix",
      headerName: <FormattedLabel id="hawkerModePrefixEn" />,
      description: <FormattedLabel id="hawkerModePrefixEn" />,
      flex: 1,
      align: "left",
      headerAlign: "center",
    },
    {
      field: "hawkerModePrefixMr",
      headerName: <FormattedLabel id="hawkerModePrefixMr" />,
      description: <FormattedLabel id="hawkerModePrefixMr" />,
      flex: 1,
      align: "left",
      headerAlign: "center",
    },
        ...
        if (res?.status == 200 || res?.status == 201) {
          ...
          let _res = response.map((r, i) => {
            return {
              id: r?.id,
              srNo: i + 1,
              hawkerModePrefix: r?.hawkerModePrefix,
              hawkerModePrefixMr: r?.hawkerModePrefixMr,
              toDate: moment(r?.toDate, "YYYY-MM-DD").format("YYYY-MM-DD"),
              fromDate: moment(r?.fromDate, "YYYY-MM-DD").format("YYYY-MM-DD"),
              hawkerMode: r.hawkerMode,
              hawkerModeMr: r?.hawkerModeMr,
              hawkerType: r?.hawkerType,
              hawkerTypeEn: hawkerTypes?.find((obj) => obj?.id === r.hawkerType)?.hawkerType,
              hawkerTypeMr: hawkerTypes?.find((obj) => obj?.id === r.hawkerType)?.hawkerTypeMr,
              remark: r?.remark,
  ...
  const onSubmitForm = (fromData) => {
    ...
    // Save - DB
    axios
      .post(`${urls.HMSURL}/hawkerMode/save`, finalBodyForApi)
      .then((res) => {
        if (res.status == 200) {
          fromData.id
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.HMSURL}/hawkerMode/save`, body).then((res) => {
            if (res.status == 200) {
    ...
    <>
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          >
            <strong>{<FormattedLabel id="hawkerMode" />}</strong>
          </div>
          {isOpenCollapse && (
                  ...
                  <form onSubmit={handleSubmit(onSubmitForm)}>
                    <Grid container style={{ marginBottom: "7vh" }}>
                      ...
                      >
                        <TextField
                          autoFocus
                          label=<FormattedLabel id="hawkerModePrefixEn" />
                          variant="standard"
                          {...register("hawkerModePrefix")}
                          error={!!errors?.hawkerModePrefix}
                          helperText={errors?.hawkerModePrefix ? errors?.hawkerModePrefix?.message : null}
                        />
                      </Grid>
                      <Grid
                        item
                      ...
                      >
                        <TextField
                          autoFocus
                          label=<FormattedLabel id="hawkerModePrefixMr" />
                          variant="standard"
                          {...register("hawkerModePrefixMr")}
                          error={!!errors?.hawkerModePrefixMr}
                          helperText={errors?.hawkerModePrefixMr ? errors?.hawkerModePrefixMr?.message : null}
                        />
                      </Grid>
                      <Grid
                        item
                      ...
                      >
                        <FormControl sx={{ marginTop: 2 }} variant="standard" error={!!errors?.hawkerType}>
                      ...
                      >
                        <TextField
                          ...
                          helperText={errors?.hawkerMode ? errors.hawkerMode.message : null}
                        />
                      </Grid>
                      <Grid
                        item
                      ...
                      >
                        <TextField
                          ...
                          helperText={errors?.hawkerModeMr ? errors?.hawkerModeMr?.message : null}
                        />
                      </Grid>
                      <Grid
                        item
