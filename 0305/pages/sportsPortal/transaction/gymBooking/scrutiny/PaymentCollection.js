...
const Index = (props) => {
  ...
  useEffect(() => {
    axios.get(`${urls.CFCURL}/master/servicecharges/getByServiceId?serviceId=36`).then((r) => {
      setChargePerCopy(r.data.serviceCharge);
    });
  }, []);
  ...
  const handlePay = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        router.push({
          pathname: "/sportsPortal/transaction/gymBooking/scrutiny/ServiceChargeRecipt",
          query: {
  ...
  useEffect(() => {
    axios.get(`${urls.SPURL}/gymBooking/getById?id=${props?.data?.id}`).then((res) => {
      reset(res.data);
      setDataa(res.data);
