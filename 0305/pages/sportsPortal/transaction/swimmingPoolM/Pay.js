...
const Index = (props) => {
  ...
  const handlePay = () => {
    ...
    const finalBody = {
      ...
      role: "LOI_COLLECTION",
      loi: getValues("loi"),
      bookingId: Number(dataa?.bookingId),
      paymentCollection: getValues("payment"),
    };

    axios
      .post(`${urls.SPURL}/swimmingBookingMonthly/saveApplicationApprove`, finalBody)
      .then((res) => {
  ...
  useEffect(() => {
    axios.get(`${urls.SPURL}/swimmingBookingMonthly/getById?id=${props?.data?.id}`).then((res) => {
      reset(res.data);
      setDataa(res.data);
