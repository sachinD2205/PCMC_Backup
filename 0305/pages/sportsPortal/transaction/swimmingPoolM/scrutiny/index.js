...
import VerificationAppplicationDetails from "../../components/VerificationAppplicationDetails";
...
const Index = () => {
  ...
  const remarkFun = (data) => {
    ...
    axios
      .post(`${urls.SPURL}/swimmingBookingMonthly/saveApplicationApprove`, finalBodyForApi, {
        headers: {
  ...
  const getSwimmingData = () => {
    axios.get(`${urls.SPURL}/swimmingBookingMonthly/getAll`).then((resp) => {
      // console.log("Swimming Data", resp);
      console.log("Swimming Data", resp.data.swimmingBooking);
      ...
      let _data = resp.data.swimmingBooking;
      ...
      setDataSource(resp.data.swimmingBooking);
      setTableData(resp.data.swimmingBooking);
      reset(resp.data.swimmingBooking);
    });
  };
      ...
      renderCell: (record) => {
        return (
          <>
            ...
            {["APPLICATION_SENT_TO_HOD", "APPLICATION_SENT_BACK_TO_HOD"].includes(
              record?.row?.applicationStatus,
            ) &&
              authority?.find((r) => r === "HOD" || r === "ADMIN") && (
                <IconButton>
                  <Button
                    variant="contained"
                    //                   // endIcon={<DoneAllIcon />}
