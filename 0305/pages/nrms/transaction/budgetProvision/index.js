...
const BudgetProvision = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: <FormattedLabel id="budgetAmountAfterCurrentBillPaymentDeduction" />,
      flex: 1,
    },
    {
      field: "budgetAmountAfterPriviousBillPaymentDeduction",
      headerName: <FormattedLabel id="budgetAmountAfterPreviousBillPaymentDeduction" />,
      flex: 1,
    },
    {
            ...
            <IconButton
              ...
              onClick={() => {
                ...
                console.log(
                  "params.row: ",
                  params.row,
                  moment(params?.row?.billAppovalDate, "DD/MM/YYYY").format("YYYY-MM-DD"),
                );
                reset(params.row);
                setValue(
                  "billAppovalDate",
  ...
  // get EventManagement
  const getBudgetProvision = (_pageSize = 10, _pageNo = 0, _sortBy = "id", _sortDir = "desc") => {
    axios
      .get(`${urls.NRMS}/trnBudgetProvision/getAll`, {
        // .get(`http://192.168.68.145:9004/nrms/api/trnBudgetProvision/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
          sortBy: _sortBy,
      ...
      .then((res) => {
        ...
        let _res = resp?.map((val, id) => {
          ...
          return {
            ...
            billAppovalDate: val.billAppovalDate ? moment(val.billAppovalDate).format("DD/MM/YYYY") : "-",
            id: val.id,
            ...
            budgetAmountAfterCurrentBillPaymentDeduction: val.budgetAmountAfterCurrentBillPaymentDeduction,
            budgetAmountAfterPriviousBillPaymentDeduction: val.budgetAmountAfterPriviousBillPaymentDeduction,
  ...
  const onSubmitForm = (formData) => {
    ...
    if (btnSaveText === "Save") {
      ...
      axios.post(`${urls.NRMS}/trnBudgetProvision/save`, _body).then((res) => {
        // axios.post(`http://192.168.68.145:9004/nrms/api/trnBudgetProvision/save`, _body).then((res) => {
        console.log("res---", res);
        if (res.status == 201) {
          getBudgetProvision();
      ...
      axios.post(`${urls.NRMS}/trnBudgetProvision/save`, _body).then((res) => {
        ...
        if (res.status == 201) {
          getBudgetProvision();
          sweetAlert("Updated!", "Record Saved successfully !", "success");
        }
      });
    }
  };
                  ...
                  <Grid container sx={{ padding: "10px" }}>
                    ...
                    >
                      <TextField
                        ...
                        helperText={errors?.budgetProvision ? errors.budgetProvision.message : null}
                      />
                    </Grid>
                    <Grid
                      item
                    ...
                    >
                      <TextField
                        ...
                        InputProps={{ style: { fontSize: 18 } }}
                        helperText={errors?.billDescription ? errors.billDescription.message : null}
                      />
                    </Grid>
                    <Grid
                      item
                    ...
                    >
                      <TextField
                        fullWidth
                        sx={{ width: "80%" }}
                        label={<FormattedLabel id="budgetAmountAfterPreviousBillPaymentDeduction" />}
                        id="standard-basic"
                        variant="standard"
                        {...register("budgetAmountAfterPriviousBillPaymentDeduction")}
                        InputLabelProps={{ shrink: true }}
                        error={!!errors.budgetAmountAfterPriviousBillPaymentDeduction}
                        InputProps={{ style: { fontSize: 18 } }}
                        helperText={
                          errors?.budgetAmountAfterPriviousBillPaymentDeduction
                            ? errors.budgetAmountAfterPriviousBillPaymentDeduction.message
                            : null
                        }
                      />
                    </Grid>
                    ...
                    >
                      <TextField
                        fullWidth
                        sx={{ width: "80%" }}
                        label={<FormattedLabel id="budgetAmountAfterCurrentBillPaymentDeduction" />}
                        id="standard-basic"
                        variant="standard"
                        {...register("budgetAmountAfterCurrentBillPaymentDeduction")}
                        InputLabelProps={{ shrink: true }}
                        error={!!errors.budgetAmountAfterCurrentBillPaymentDeduction}
                        InputProps={{ style: { fontSize: 18 } }}
                        helperText={
                          errors?.budgetAmountAfterCurrentBillPaymentDeduction
                            ? errors.budgetAmountAfterCurrentBillPaymentDeduction.message
                            : null
                        }
                      />
                    </Grid>
                    ...
                    >
                      <FormControl error={!!errors.billAppovalDate} fullWidth sx={{ width: "80%" }}>
                        <Controller
                          control={control}
                        ...
                        />
                        <FormHelperText>
                          {errors?.billAppovalDate ? errors.billAppovalDate.message : null}
                        </FormHelperText>
                      </FormControl>
                    </Grid>
                    <Grid
                    ...
                    >
                      <TextField
                        ...
                        InputProps={{ style: { fontSize: 18 } }}
                        helperText={
                          errors?.billAppovalOfficerName ? errors.billAppovalOfficerName.message : null
                        }
                      />
                    </Grid>
                    <Grid
