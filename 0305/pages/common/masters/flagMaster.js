...
const FlagMaster = () => {
  ...
  const getFlag = (_pageSize = 10, _pageNo = 0, _sortBy = "id", _sortDir = "desc") => {
    ...
    axios
      ...
      .then((response) => {
        let result = response.data.flagName;
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.CFCURL}/master/flagMaster/save`, body).then((res) => {
  ...
  const onSubmitForm = (formData) => {
    ...
    axios.post(`${urls.CFCURL}/master/flagMaster/save`, finalBodyForApi).then((res) => {
      ...
      <Paper style={{ margin: "50px" }}>
        ...
        <Box style={{ height: "auto", overflow: "auto" }}>
          <DataGrid
            ...
            columns={columns}
            onPageChange={(_data) => {
              getFlag(data.pageSize, _data);
            }}
            onPageSizeChange={(_data) => {
              console.log("222", _data);
              // updateData("page", 1);
              getFlag(_data, data.page);
            }}
          />
        </Box>
      </Paper>
