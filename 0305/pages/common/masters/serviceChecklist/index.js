...
const ServiceChecklist = () => {
  ...
  useEffect(() => {
    getServiceChecklist();
  }, [serviceList, departmentList, documentList]);
  ...
  const getServiceChecklist = async (_pageSize = 10, _pageNo = 0, _sortBy = "id", _sortDir = "desc") => {
    setLoad(true);
      ...
      .then((res) => {
        ...
        let _res = result.map((val, i) => {
          ...
          return {
            ...
            // documentChecklistEn: val.documentChecklistEn,
            service: val.service,
            serviceCol: serviceList.find((f) => f.id === val.service)?.serviceName,
            usageType: val.usageType,
            usageTypeCol: usageTypeList.find((f) => f.id === val.usageType)?.usageType,
            document: val.document,
            documentCol: documentList.find((f) => f.id === val.document)?.documentChecklistEn,
            documentType: val.documentType,
            // documentChecklistEn: val.documentChecklistEn,
            // documentChecklistMr: val.documentChecklistMr,
            isDocumentMandetoryState: val.isDocumentMandetory === true ? "true" : "false",
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.BaseURL}/serviceWiseChecklist/save`, body).then((res) => {
  ...
  const onSubmitForm = (formData) => {
    ...
    axios.post(`${urls.CFCURL}/master/serviceWiseChecklist/save`, finalBodyForApi).then((res) => {
      if (res.status == 200) {
        console.log("11id", id);
          ...
          <Slide direction="down" in={slideChecked} mountOnEnter unmountOnExit>
            <form onSubmit={handleSubmit(onSubmitForm)}>
              <Grid container style={{ padding: "10px" }}>
                ...
                >
                  <FormControl size="small" sx={{ m: 1, width: "86%" }}>
                    <InputLabel id="demo-simple-select-standard-label">Usage Type (Document Type)</InputLabel>
                    ...
                    <Controller
                      render={({ field }) => (
                        <Select
                          labelId="demo-simple-select-label"
                          id="demo-simple-select"
                          label="Usage Type (Document Type)"
                          value={field.value}
                          // onChange={(value) => field.onChange(value)}
                          onChange={(value) => {
                            field.onChange(value);
              ...
              <Grid container style={{ padding: "10px" }}>
                ...
                >
                  <FormControl size="small" sx={{ m: 1, width: "86%" }}>
                    <InputLabel id="demo-simple-select-standard-label">Document Name</InputLabel>
                            ...
                            ? documentList.map((document, index) => {
                                return (
                                  <MenuItem key={index} value={document.id}>
                                    ...
                                    {lang === "en" ? document.document : document.documentChecklistMr}
                                  </MenuItem>
                                );
                              })
                            : "NA"}
