...
function DepositRefund() {
  ...
  const onSubmitFunc = (data) => {
    ...
    if (watch("fromDate") && watch("toDate")) {
      ...
      setLoading(true);
      axios
        .get(`${urls.PABBMURL}/trnDepositeRefundProcessByDepartment/getReportByDateAndAuditoriumId`, {
          params: {
            fromDate: fromDate,
            toDate: toDate,
        ...
        .then((res) => {
          ...
          if (res?.status === 200 || res?.status === 201) {
            if (res?.data?.trnDepositeRefundProcessByDepartmentList?.length > 0) {
              let _res = res?.data?.trnDepositeRefundProcessByDepartmentList?.map((r, i) => {
                ...
                return {
                  ...
                  loiNo: r?.trnAuditoriumBookingOnlineProcess?.LoiNo
                    ...
                    : "-",
                  applicantMobile: r?.trnAuditoriumBookingOnlineProcess?.applicantMobileNo,
    ...
    <Paper>
      ...
      >
        ...
        <Grid item xs={8} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
          <h2>{language === "en" ? "Deposit Refund Report" : "ठेव परतावा अहवाल"}</h2>
        </Grid>
        {/* <Grid item xs={2} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
          <Button
            size="small"
                    ...
                    render={({ field }) => (
                      <LocalizationProvider dateAdapter={AdapterMoment}>
                        <DatePicker
                          ...
                          center
                          renderInput={(params) => (
                            <TextField {...params} size="small" fullWidth error={errors.fromDate} />
                          )}
                        />
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <LocalizationProvider dateAdapter={AdapterMoment}>
                        <DatePicker
                          inputFormat="DD/MM/YYYY"
                          label={<span> {language === "en" ? "To Date" : "पर्यंत"}</span>}
                          value={field.value}
                          onChange={(date) => field.onChange(date)}
                          selected={field.value}
                          center
                          ...
                          renderInput={(params) => (
                            <TextField {...params} error={errors.fromDate} size="small" fullWidth />
                          )}
                        />
                      </LocalizationProvider>
                    )}
