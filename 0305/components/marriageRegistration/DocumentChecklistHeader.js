...
const Index = () => {
  ...
  const viewForm = (props) => {
    ...
    if (serviceId === 10) {
      ...
      axios
        .get(`${urls.MR}/transaction/marriageBoardRegistration/getapplicantById?applicationId=${ID}`, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        })
  ...
  useEffect(() => {
    ...
    setValue("serviceName", language == "en" ? ValueServiceName : ValueServiceNameMr);
    setValue(
      "applicantName",
      language === "en" ? router?.query?.applicantName : router?.query?.applicantNameMr,
    );
  }, [language]);
      ...
      <ThemeProvider theme={theme}>
          ...
          <div className={styles.small}>
            <div className={styles.detailsApot}>
              <div className={styles.h1TagApot}>
                ...
                >
                  {language === "en" ? router?.query?.pageHeader : router?.query?.pageHeaderMr}
                  {/* {
            ...
            <div className={styles.row2}>
              ...
              <div>
                ...
                <FormControl variant="standard" style={{ marginTop: 10 }} error={!!errors.applicationDate}>
                  <Controller
                    control={control}
        ...
        <Dialog fullWidth maxWidth={"lg"} open={formPreviewDailog} onClose={() => formPreviewDailogClose()}>
          ...
          <DialogContent>
            ...
            {serviceId === 67 ? (
              <>
                <BoardRegistration onlyDoc={false} preview={true} photos={data ? data : []} />
              </>
            ) : (
              ""
            )}
            {serviceId === 15 ? (
              <>
                <ModBoardRegistration onlyDoc={false} preview={true} photos={data ? data : []} />
              </>
            ) : (
              ""
            )}
            ...
            {serviceId === 12 ? (
              <>
                <ModMarriageCertificate onlyDoc={false} preview={true} photos={data ? data : []} />
              </>
            ) : (
              ""
            )}
